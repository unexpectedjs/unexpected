/*global expect*/
describe('to be fulfilled with a value satisfying assertion', function () {
    ...
    it('should succeed if the response is resolved with a reason satisfying the argument', function () {
        ...
        }), 'to be fulfilled with a value satisfying', 123);
    });

    it('should forward the fulfillment value', function () {
        return expect(expect.promise.resolve(123), 'to be fulfilled with a value satisfying', 123).then(function (value) {
    ...
    it('should fail if the promise is resolved with a value that does not satisfy the argument', function () {
        return expect(
            ...
            }), 'to be fulfilled with a value satisfying', { baz: 'qux' }),
            'to be rejected with',
            "expected Promise to be fulfilled with a value satisfying { baz: 'qux' }\n" +
    ...
    describe('with the "exhaustively" flag', function () {
        it('errors if the expected value doesn\'t contain all the values in the subject', function () {
            return expect(
                expect(Promise.resolve({
                    ...
                    bar: 'bar',
                    quux: 'quux'
                }), 'to be fulfilled with a value exhaustively satisfying', {
                    foo: 'foo',
                    bar: 'bar'
                }),
                'to be rejected'
        ...
        it('errors with the correct error', function () {
            return expect(
                ...
                'to be rejected with',
                "expected Promise\n" +
                "to be fulfilled with a value exhaustively satisfying { foo: 'foo', bar: 'bar' }\n" +
    ...
    describe('without the "exhaustively" flag', function () {
        it('doesn\'t error if the expected value doesn\'t contain all the values in the subject', function () {
            return expect(expect.promise.resolve({
                ...
                bar: 'bar',
                quux: 'quux'
            }), 'to be fulfilled with a value satisfying', {
                foo: 'foo',
                bar: 'bar'
            });
        });
        ...
        it('should fail if the function returns a promise that is fulfilled with the wrong value', function () {
            expect(
                function () {
                    ...
                    }, 'to be fulfilled with a value satisfying', 456);
                },
                'to throw',
                "expected\n" +
                "function () {\n" +

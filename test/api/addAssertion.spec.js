...
describe('addAssertion', () => {
  it('is chainable', () => {
    expect.addAssertion('foo', () => {}).addAssertion('bar', () => {});
  ...
  it('supports transfering flags from the custom assertion to nested expect', () => {
    var clonedExpect = expect
      .clone()
      .addAssertion('[not] to be sorted', (expect, subject) => {
  ...
  describe('when overriding an assertion', () => {
    it('uses the most specific version', () => {
      var clonedExpect = expect
        ...
        .addAssertion('<any> to foo', (expect, subject) => {
          expect.errorMode = 'bubble';
          expect.fail('new');
        });
  ...
  it('allows overlapping patterns within a single addAssertion call', () => {
    expect(() => {
      expect.clone().addAssertion(['to foo', 'to [really] foo'], () => {});
    }, 'not to throw');
  });
  ...
  describe('pattern', () => {
    ...
    describe('alternations', () => {
      it('can be empty', () => {
        var clonedExpect = expect
          .clone()
          .addAssertion('to foo (|bar)', (expect, subject) => {
  ...
  describe('types', () => {
    ...
    it('allows specifying assertions with overlapping patterns for different types', () => {
      ...
      clonedExpect
        ...
        .addAssertion('<box> to be foo', (expect, subject) => {
          expect(subject.value, 'to be', 'foo');
        })
        .addAssertion('<string> to be foo', (expect, subject) => {
          expect(subject, 'to be', 'foo');
        })
        .addAssertion('<any> to be foo', (expect, subject) => {
  ...
  describe('error modes', () => {
    ...
    describe('for synchronous custom assertions', () => {
      ...
      it('avoids repeating large subjects', () => {
        var clonedExpect = expect
          .clone()
          .addAssertion('to foobarbaz', (expect, subject) => {
    ...
    describe('for asynchronous custom assertions', () => {
      beforeEach(() => {
        clonedExpect = expect
          .clone()
          .addAssertion(
            'to be sorted after delay',
            (expect, subject, delay, done) => {
              expect.errorMode = errorMode;
      ...
      it('errorMode=nested nest the error message of expect failures in the assertion under the assertion standard message', done => {
        errorMode = 'nested';
        clonedExpect(42, 'to be sorted after delay', 1, err => {
          expect(
            err,
            'to have message',
      ...
      it('errorMode=bubble bubbles uses the error message of expect failures in the assertion', done => {
        errorMode = 'bubble';
        clonedExpect(42, 'to be sorted after delay', 1, err => {
      ...
      it('errorMode=diff only includes the diff', done => {
        errorMode = 'diff';
        clonedExpect([3, 2, 1], 'to be sorted after delay', 1, err => {
          expect(
            err,
            'to have message',
            '[\n' +
      ...
      it('errorMode=default uses the standard error message of the assertion', done => {
        errorMode = 'default';
        clonedExpect(42, 'to be sorted after delay', 1, err => {
          expect(
            err,
    ...
    describe('when the error mode of the assertion changes after the assertion has failed', () => {
      it('serializes the error with the error mode that was in effect at the time of its creation', () => {
        var clonedExpect = expect
          .clone()
          .addAssertion('to be equal to foo', (expect, subject) => {
            expect.errorMode = 'nested';
            try {
              expect(subject, 'to equal', 'foo');
            } catch (e) {
  ...
  it('nested expects throws if the assertion does not exists', () => {
    var clonedExpect = expect
      .clone()
      .addAssertion('to be foo', (expect, subject) => {
  ...
  it('throws if the first parameter is not a string or an array', () => {
    expect(
      () => {
        expect.addAssertion(123, (expect, subject) => {});
      },
      'to throw',
  ...
  it('makes expect.it available inside a custom assertion', () => {
    var clonedExpect = expect.clone();
    clonedExpect.addAssertion('to foo', (expect, subject) => {

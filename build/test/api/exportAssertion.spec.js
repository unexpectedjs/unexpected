...
describe('exportAssertion', function () {
    ...
    it('is chainable', function () {
        childExpect.exportAssertion('foo', function () {}).exportAssertion('bar', function () {});
    ...
    it('does not make the assertion available to a parent parent expect', function () {
        childExpect.child().exportAssertion('<string> to foo', function (expect, subject) {
            expect(subject, 'to equal', 'foo');
        });
        expect(function () {
            parentExpect('foo', 'to foo');
        }, 'to throw', /^Unknown assertion 'to foo'/);
    });
    ...
    it('binds the assertion to the child expect so custom types are available', function () {
        ...
        }, 'to throw', 'expected >>yaddafo<< to be silly\n' + '\n' + 'yaddafo\n' + '     ^^');
    });
            ...
            if (subject !== 'foo') {
                expect.fail({
                    diff: function diff(output, _diff, inspect, equal) {
                        return output.text('got ').fancyQuotes(subject).text(' but expected ').fancyQuotes('foo').text(' and once again ').append(inspect(subject)).text(' the difference being').nl().append(_diff(subject, 'foo')).nl().text('and they really are different, see: ' + equal(subject, 'foo'));
                    }
                });
            }
        });
    ...
    it('should make custom styles available when a parent expect appends an error associated with a child expect', function () {
        ...
        childExpect.exportAssertion('<any> to foo', function (expect, subject) {
            if (subject !== 'foo') {
                expect.fail({
                    diff: function diff(output, _diff2, inspect, equal) {
                        return output.text('got ').fancyQuotes(subject).text(' but expected ').fancyQuotes('foo');
                    }
                });
            }
        });
    ...
    describe('when shifting from an exported middle-of-the-rocket assertion in an inherited expect to an assertion defined in the parent expect', function () {
        ...
        it('should fail with a diff', function () {
            ...
            }, 'to throw', "expected 'bar' when prepended with foo to foo\n" + "\n" + "-foobar\n" + "+foo");
        });
    });
    ...
    describe('when delegating to an exported assertion from the parent', function () {
        beforeEach(function () {
            ...
            childExpect.exportAssertion('<string> when trimmed <assertion?>', function (expect, subject) {
        ...
        it('should fail with a diff', function () {
            expect(function () {
                parentExpect('bar', 'when prepended with foo when prepended with foo', 'to foo');
            }, 'to throw', "expected 'bar' when prepended with foo when prepended with foo to foo\n" + "\n" + "-foofoobar\n" + "+foo");
        });
    });
});
...
describe('addType', function () {
    ...
    it('throws an expection if the base type does not exist', function () {
        expect(function () {
            clonedExpect.addType({ name: 'foo', base: 'barquux', identify: function identify() {
                    return false;
                } });
    ...
    describe('with a custom box type', function () {
            ...
            beforeEach(function () {
                clonedExpect.addType({
                    ...
                    identify: function identify(obj) {
                        return obj && (typeof obj === 'undefined' ? 'undefined' : _typeof(obj)) === 'object' && obj.isBox;
                    },
                    equal: function equal(a, b, _equal) {
                        return a === b || _equal(a.value, b.value);
                    },
                    inspect: function inspect(obj, depth, output, _inspect) {
                        return output.text('box(').append(_inspect(obj.value)).text(')');
                    },
                    diff: function diff(actual, expected, output, _diff) {
                        output = output.text('box(').append(_diff({ value: actual.value }, { value: expected.value })).text(')');
                        output.inline = true;
                        return output;
                    }
                });
        ...
        describe('added with a base type of wrapperObject', function () {
            ...
            it('should include the diff when one is available', function () {
                ...
                }, 'to throw', "expected box('abc') to equal box('abe')\n" + "\n" + "box(\n" + "  'abc' // should equal 'abe'\n" + "        //\n" + "        // -abc\n" + "        // +abe\n" + ")");
            });
        });
        ...
        it('allows adding a type whose diff method returns an old-style { inline: <boolean>, diff: <magicpen> } object', function () {
            clonedExpect.addType({
                ...
                equal: function equal(a, b, _equal2) {
                    return a === b || _equal2(a.value, b.value);
                },
                inspect: function inspect(obj, depth, output, _inspect2) {
                    return output.text('box(').append(_inspect2(obj.value)).text(')');
                },
                diff: function diff(actual, expected, output, _diff2) {
                    return {
                        inline: true,
                        diff: output.text('box(').append(_diff2({ value: actual.value }, { value: expected.value })).text(')')
                    };
                }
            });
    ...
    describe('base type', function () {
        describe('#inspect', function () {
            ...
            it('inspects a value', function () {
                expect(clonedExpect.getType('number').baseType.inspect(null, 3, clonedExpect.createOutput()).toString(), 'to equal', 'null');
            });
            ...
            it('provides an inspect function as the 4th parameter', function () {
                clonedExpect.addType({
                    ...
                    inspect: function inspect(value, depth, output, _inspect3) {
                        return output.append(_inspect3('foo'));
                    }
                });
                ...
                clonedExpect.addType({
                    ...
                    inspect: function inspect(value, depth, output, _inspect4) {
                        return _inspect4(value);
                    }
                });
                expect(clonedExpect.getType('bar').baseType.inspect(null, 3, clonedExpect.createOutput(), function (value) {
                    return expect.createOutput().appendInspected(value);
                }).toString('text'), 'to equal', "'foo'");
            });
        });

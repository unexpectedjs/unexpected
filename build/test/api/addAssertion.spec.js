...
describe('addAssertion', function () {
    it('is chainable', function () {
        expect.addAssertion('foo', function () {}).addAssertion('bar', function () {});
    ...
    it('supports transfering flags from the custom assertion to nested expect', function () {
        var clonedExpect = expect.clone().addAssertion('[not] to be sorted', function (expect, subject) {
    ...
    describe('when overriding an assertion', function () {
        it('uses the most specific version', function () {
            var clonedExpect = expect.clone().addAssertion('<string> to foo', function (expect, subject) {
    ...
    it('does not break when declaring multiple patterns that do not have the same set of flags defined', function () {
        var clonedExpect = expect.clone().addAssertion(['[not] to be foo', 'to be foo aliased without the not flag'], function (expect, subject) {
    ...
    describe('types', function () {
        ...
        it('allows specifying assertions with overlapping patterns for different types', function () {
            ...
            clonedExpect.addType({
                ...
                inspect: function inspect(box, depth, output, _inspect) {
                    output.text('[Box ').append(_inspect(box.value)).text(']');
                    return output;
                }
        ...
        it('allows you to control the inspection depth', function () {
            var clonedExpect = expect.clone().addType({
                ...
                inspect: function inspect(box, depth, output, _inspect2) {
                    output.text('[Box ').append(_inspect2(box.value, depth)).text(']');
                    return output;
                }
            });

            expect(clonedExpect.inspect(new Box({ shown: { hidden: 'secret' } }), 1).toString(), 'to equal', '[Box { shown: ... }]');
        });
    });
    ...
    describe('error modes', function () {
        ...
        describe('for synchronous custom assertions', function () {
            beforeEach(function () {
                clonedExpect = expect.clone().addAssertion('[not] to be sorted', function (expect, subject) {
            ...
            it('errorMode=bubble only includes the diff once', function () {
                ...
                expect(function () {
                    clonedExpect([3, 2, 1], 'to be sorted');
                }, 'to throw', 'expected [ 3, 2, 1 ] to equal [ 1, 2, 3 ]\n' + '\n' + '[\n' + '┌───▷\n' + '│ ┌─▷\n' + '│ │   3,\n' + '│ └── 2, // should be moved\n' + '└──── 1 // should be moved\n' + ']');
            });
        ...
        describe('for asynchronous custom assertions', function () {
            beforeEach(function () {
                clonedExpect = expect.clone().addAssertion('to be sorted after delay', function (expect, subject, delay, done) {
                    expect.errorMode = errorMode;
        ...
        describe('for custom assertions that return promises', function () {
            ...
            it('errorMode=nested nest the error message of expect failures in the assertion under the assertion standard message', function () {
                errorMode = 'nested';
                return expect(clonedExpect(42, 'to be sorted after delay', 1), 'to be rejected with', 'expected 42 to be sorted after delay 1\n  expected 42 to be an array');
            });

            it('errorMode=bubble bubbles uses the error message of expect failures in the assertion', function () {
                errorMode = 'bubble';
                return expect(clonedExpect(42, 'to be sorted after delay', 1), 'to be rejected with', 'expected 42 to be an array');
            });
            ...
            describe('nested inside another custom assertion', function () {
                ...
                it('errorMode=diff only includes the diff', function () {
                    errorMode = 'diff';
                    return expect(clonedExpect([3, 2, 1], 'to be sorted after delay', 1), 'to be rejected with', '[\n' + '┌───▷\n' + '│ ┌─▷\n' + '│ │   3,\n' + '│ └── 2, // should be moved\n' + '└──── 1 // should be moved\n' + ']');
                });
        ...
        describe('when the error mode of the assertion changes after the assertion has failed', function () {
            it('serializes the error with the error mode that was in effect at the time of its creation', function () {
                ...
                }, 'to throw', "expected 'bar' to be equal to foo\n" + "  expected 'bar' to equal 'foo'\n" + "\n" + "  -bar\n" + "  +foo");
            });
        });
    });
    ...
    it('throws if the first parameter is not a string or an array', function () {
        ...
        }, 'to throw', 'Syntax: expect.addAssertion(<string|array[string]>, function (expect, subject, ...) { ... });');
    });
    ...
    it('makes expect.it available inside a custom assertion', function () {
        ...
        expect(function () {
            clonedExpect('bar', 'to foo');
        }, 'to throw', "expected 'bar' to equal 'foo'\n" + '\n' + '-bar\n' + '+foo');
    });
});
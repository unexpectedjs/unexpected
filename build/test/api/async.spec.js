...
describe('async', function () {
    ...
    var clonedExpect = expect.clone().addAssertion('to be sorted after delay', function (expect, subject, delay) {
    ...
    it('supports composition', expect.async(function () {
        return expect(clonedExpect([1, 3, 2], 'to be ordered after delay'), 'to be rejected with', 'expected [ 1, 3, 2 ] to be ordered after delay\n' + '  expected [ 1, 3, 2 ] to be sorted after delay 20\n' + '    expected [ 1, 3, 2 ] to equal [ 1, 2, 3 ]\n' + '\n' + '    [\n' + '        1,\n' + '    ┌─▷\n' + '    │   3,\n' + '    └── 2 // should be moved\n' + '    ]');
    }));

    it('has a nice syntax', expect.async(function () {
        return expect(clonedExpect([1, 3, 2], 'to be sorted after delay', 20), 'to be rejected with', 'expected [ 1, 3, 2 ] to be sorted after delay 20\n' + '  expected [ 1, 3, 2 ] to equal [ 1, 2, 3 ]\n' + '\n' + '  [\n' + '      1,\n' + '  ┌─▷\n' + '  │   3,\n' + '  └── 2 // should be moved\n' + '  ]');
    }));
    ...
    if (workQueue) {
        it('throw an unhandled rejection if a promise is not caught by the test', function (done) {
            workQueue.onUnhandledRejection = function (err) {
                expect(err.getErrorMessage({ format: 'text' }).toString(), 'to equal', 'expected [ 1, 3, 2 ] to be ordered after delay\n' + '  expected [ 1, 3, 2 ] to be sorted after delay 20\n' + '    expected [ 1, 3, 2 ] to equal [ 1, 2, 3 ]\n' + '\n' + '    [\n' + '        1,\n' + '    ┌─▷\n' + '    │   3,\n' + '    └── 2 // should be moved\n' + '    ]');

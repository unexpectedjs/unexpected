...
describe('exportType', function () {
    ...
    it('is chainable', function () {
        childExpect.exportType({ name: 'abc', identify: false }).exportType({ name: 'def', identify: false });
    ...
    it('binds the type to the child expect so custom types are available to the diff function', function () {
        ...
        childExpect.exportType({
            ...
            diff: function diff(actual, expected, output) {
                output.text('got ').fancyQuotes(actual).text(' but expected ').fancyQuotes(expected);
            }
        });
        expect(parentExpect.getType('yadda').diff('yaddafoo', 'yaddabar', parentExpect.createOutput('text').text('hey, ')).toString(), 'to equal', 'hey, got >>yaddafoo<< but expected >>yaddabar<<');
    });
    ...
    it('should correctly append output generated by a inspect function that does not return the output', function () {
        ...
        expect(parentExpect.createOutput().appendInspected('yaddafoo').toString(), 'to equal', '>>yaddafoo<<yaddafo>>yaddafo<<yaddaf>>yaddaf<<yadda>>yadda<<');
    });

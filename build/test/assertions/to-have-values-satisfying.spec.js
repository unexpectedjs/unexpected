...
describe('to have values satisfying assertion', function () {
    ...
    it('fails if the given object is empty', function () {
        ...
        }, 'to throw', "expected {} to have values satisfying\n" + "function (value) {\n" + "  expect(value, 'to equal', '0');\n" + "}\n" + "  expected {} not to be empty");
    });

    it('fails for an empty array', function () {
        expect(function () {
            expect([], 'to have values satisfying', 123);
        }, 'to throw', "expected [] to have values satisfying 123\n" + "  expected [] not to be empty");
    });
    ...
    it('fails if the given array is empty', function () {
        ...
        }, 'to throw', "expected [] to have items satisfying\n" + "function (item) {\n" + "  expect(item, 'to be a number');\n" + "}\n" + "  expected [] not to be empty");
    });
    ...
    describe('delegating to an async assertion', function () {
        var clonedExpect = expect.clone().addAssertion('to be a number after a short delay', function (expect, subject, delay) {
        ...
        it('should fail with a diff', function () {
            return expect(clonedExpect({ 0: 0, 1: false, 2: 'abc' }, 'to have values satisfying', 'to be a number after a short delay'), 'to be rejected with', "expected { 0: 0, 1: false, 2: 'abc' }\n" + "to have values satisfying to be a number after a short delay\n" + "\n" + "{\n" + "  0: 0,\n" + "  1: false, // should be a number after a short delay\n" + "            //   should be a number\n" + "  2: 'abc' // should be a number after a short delay\n" + "           //   should be a number\n" + "}");
        });
    });

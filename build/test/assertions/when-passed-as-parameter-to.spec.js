...
describe('when passed as parameters to assertion', function () {
    ...
    describe('with the async flag', function () {
        ...
        it('should fail if the result of the async function does not meet the criteria', function () {
            return expect(expect([123], 'when passed as parameters to async', delayedIncrement, 'to equal', 125), 'to be rejected with', "expected [ 123 ] when passed as parameters to async\n" + "function delayedIncrement(num, cb) {\n" + "  setTimeout(function () {\n" + "    if (typeof num === 'number') {\n" + "      cb(null, num + 1);\n" + "    } else {\n" + "      cb(new Error('not a number'));\n" + "    }\n" + "  }, 1);\n" + "} to equal 125\n" + "  expected 124 to equal 125");
        });

        it('should fail if the async function calls the callback with an error', function () {
            return expect(expect([false], 'when passed as parameters to async', delayedIncrement, 'to equal', 125), 'to be rejected with', "expected [ false ] when passed as parameters to async\n" + "function delayedIncrement(num, cb) {\n" + "  setTimeout(function () {\n" + "    if (typeof num === 'number') {\n" + "      cb(null, num + 1);\n" + "    } else {\n" + "      cb(new Error('not a number'));\n" + "    }\n" + "  }, 1);\n" + "} to equal 125\n" + "  expected Error('not a number') to be falsy");
        });
    });
});
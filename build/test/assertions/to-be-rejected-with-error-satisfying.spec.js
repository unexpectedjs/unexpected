...
describe('to be rejected with error satisfying assertion', function () {
    ...
    it('should provide the rejection reason as the fulfillment value', function () {
        return expect(expect.promise.reject(new Error('foo')), 'to be rejected with error satisfying', 'foo').then(function (reason) {
    ...
    it('should fail if the promise is rejected with a reason that does not satisfy the argument', function () {
        return expect(expect(new Promise(function (resolve, reject) {
            setTimeout(function () {
                reject(new Error('OMG!'));
            }, 1);
        }), 'to be rejected with error satisfying', new Error('foobar')), 'to be rejected with', "expected Promise to be rejected with error satisfying Error('foobar')\n" + "  expected Error('OMG!') to satisfy Error('foobar')\n" + "\n" + "  Error({\n" + "    message: 'OMG!' // should equal 'foobar'\n" + "                    //\n" + "                    // -OMG!\n" + "                    // +foobar\n" + "  })");
    });
    ...
    describe('with the "exhaustively" flag', function () {
        it('errors if the rejection reason doesn\'t have all the same properties as the value', function () {
            return expect(function () {
                ...
                error.data = { foo: 'bar' };
                return expect(expect.promise.reject(error), 'to be rejected with error exhaustively satisfying', new Error('foobar'));
            }, 'to error');
        });
    ...
    describe('without the "exhaustively" flag', function () {
        it('does not error if the rejection reason doesn\'t have all the same properties as the value', function () {
            return expect(function () {
                var error = new Error('foobar');
                error.data = { foo: 'bar' };
                return expect(expect.promise.reject(error), 'to be rejected with error satisfying', new Error('foobar'));
            }, 'not to error');
        });
    });
    ...
    describe('when passed a function as the subject', function () {
        it('should fail if the function returns a promise that is rejected with the wrong reason', function () {
            expect(function () {
                ...
                }, 'to be rejected with error satisfying', new Error('bar'));
        ...
        describe('with the "exhaustively" flag', function () {
            it('errors if the rejection reason doesn\'t have all the same properties as the value', function () {
                return expect(function () {
                    ...
                    }, 'to be rejected with error exhaustively satisfying', new Error('foobar'));
                }, 'to error');
            });

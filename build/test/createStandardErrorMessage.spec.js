...
describe('createStandardErrorMessage', function () {
    ...
    it('should not pick up the subject compaction code from a MagicPen instance in the prototype chain', function () {
        var clonedExpect = expect.clone().addAssertion('<any> shifted <assertion>', function (expect, subject) {
            expect.errorMode = 'nested';
            return expect.shift();
        }).addAssertion('<array> to be an array of one foo', function (expect, subject) {
            expect.errorMode = 'nested';
            expect(subject[0], 'to equal', 'foo');
        });
        expect(function () {
            clonedExpect(['foooooooooooooooooooooooooooooooooooo'], 'shifted to be an array of one foo');
        }, 'to throw', "expected [ 'foooooooooooooooooooooooooooooooooooo' ]\n" + "shifted to be an array of one foo\n" + "  expected array to be an array of one foo\n" + "    expected 'foooooooooooooooooooooooooooooooooooo' to equal 'foo'\n" + "\n" + "    -foooooooooooooooooooooooooooooooooooo\n" + "    +foo");
    });
});
...
UnexpectedError.prototype.getDefaultErrorMessage = function (isShort) {
    var message = this.expect.output.clone();
    if (this.assertion) {
        message.append(this.assertion.standardErrorMessage(isShort));
    } else {
...
UnexpectedError.prototype.getNestedErrorMessage = function () {
    var message = this.expect.output.clone();
    var isShort = this.parent && this.parent.subject === this.subject;
    if (this.assertion) {
    ...
    message.nl()
        .indentLines()
        .i().block(this.parent.getErrorMessage(isShort));
    return message;
};
...
UnexpectedError.prototype.getErrorMessage = function (isShort) {
    ...
    switch (errorMode) {
    case 'nested': return this.getNestedErrorMessage(isShort);
    case 'default': return this.getDefaultErrorMessage(isShort);
    case 'bubble': return this.parent.getErrorMessage(isShort);

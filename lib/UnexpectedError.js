...
const missingOutputMessage = "You must either provide a format or a magicpen instance";
  ...
  return (
    ...
    makeDiffResultBackwardsCompatible(
      this.createDiff(
        ...
        (v, depth) => output.clone().appendInspected(v, (depth || defaultDepth) - 1),
...
function findStackStart(lines) {
  for (let i = lines.length - 1; 0 <= i; i -= 1) {
    if (lines[i] === "") {
      return i + 1;
    }
  }
...
UnexpectedError.prototype.serializeMessage = function(outputFormat) {
  ...
  format: htmlFormat ? "text" : outputFormat
    ...
    if (
      ...
      const index = this.originalError.stack.indexOf(this.originalError.message);
      if (index === -1) {
        // Phantom.js doesn't include the error message in the stack property
        this.stack = `${this.message}\n${this.originalError.stack}`;
      } else {
        this.stack =
          this.message + this.originalError.stack.substr(index + this.originalError.message.length);
      }

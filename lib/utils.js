...
var utils = (module.exports = {
  ...
  uniqueNonNumericalStringsAndSymbols: function() {
    // ...
    return utils.uniqueStringsAndSymbols(stringOrSymbol => typeof stringOrSymbol === "symbol" || !utils.numericalRegExp.test(stringOrSymbol), Array.prototype.slice.call(arguments));
  },

  forwardFlags: function(testDescriptionString, flags) {
    return testDescriptionString
      .replace(/\[(!?)([^\]]+)\] ?/g, (match, negate, flag) => Boolean(flags[flag]) !== Boolean(negate) ? flag + " " : "")
      .trim();
  },
  ...
  packArrows: function(changes) {
    ...
    if (moveIndices.length > 0) {
      var arrowSpecs = [];
      moveIndices
        .sort((a, b) => // Order by distance between change indices descending
      Math.abs(

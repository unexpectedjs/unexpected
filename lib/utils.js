...
const canSetPrototype = Object.setPrototypeOf || { __proto__: [] } instanceof Array;
...
const utils = (module.exports = {
  ...
  getFunctionName: function(f) {
    ...
    const matchFunctionName = Function.prototype.toString.call(f).match(/function ([^\(]+)/);
  ...
  wrapConstructorNameAroundOutput: function(output, obj) {
    ...
    const constructorName =
      constructor && constructor !== Object && utils.getFunctionName(constructor);
  ...
  uniqueStringsAndSymbols: function(...args) {
  ...
  uniqueNonNumericalStringsAndSymbols: function(...args) {
    // ...
    return utils.uniqueStringsAndSymbols(
      stringOrSymbol =>
        typeof stringOrSymbol === "symbol" || !utils.numericalRegExp.test(stringOrSymbol),
  ...
  forwardFlags: function(testDescriptionString, flags) {
    return testDescriptionString
      .replace(
        /\[(!?)([^\]]+)\] ?/g,
        (match, negate, flag) => (Boolean(flags[flag]) !== Boolean(negate) ? `${flag} ` : "")
      )
      .trim();
  },
    ...
    changes.forEach((diffItem, index) => {
      if (diffItem.type === "moveSource") {
        ...
        (moveSourceAndTargetByActualIndex[diffItem.actualIndex] =
          moveSourceAndTargetByActualIndex[diffItem.actualIndex] || {}).source = diffItem;
      } else if (diffItem.type === "moveTarget") {
        diffItem.changeIndex = index;
        (moveSourceAndTargetByActualIndex[diffItem.actualIndex] =
          moveSourceAndTargetByActualIndex[diffItem.actualIndex] || {}).target = diffItem;
      }
    });
    ...
    if (moveIndices.length > 0) {
      ...
      moveIndices
        ...
        .forEach((actualIndex, i, keys) => {
          const moveSourceAndMoveTarget = moveSourceAndTargetByActualIndex[actualIndex];

...
var utils = module.exports = {
    ...
    uniqueNonNumericalStringsAndSymbols: function () { // array1, array2 ...
        var index = {};
        var uniqueItems = [];
        for (var i = 0 ; i < arguments.length ; i += 1) {
            arguments[i].forEach(function (stringOrSymbol) {
                var isSymbol;
                /*jshint ignore:start*/
                isSymbol = typeof stringOrSymbol === 'symbol';
                /*jshint ignore:end*/
                if (!isSymbol && !numericalRegExp.test(stringOrSymbol) && !index[stringOrSymbol]) {
                    index[stringOrSymbol] = true;
                    uniqueItems.push(stringOrSymbol);
                }
            });
        }
        return uniqueItems;

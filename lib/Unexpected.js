...
function calculateRequirementSpecificity(requireArguments) {
...
function calculateAssertionSpecificity(assertion) {
    return assertion.args.reduce(function (result, requirements) {
        return result + calculateRequirementSpecificity(requirements);
    }, calculateRequirementSpecificity(assertion.subject));
}
...
Unexpected.prototype.throwAssertionNotFoundError = function (subject, testDescriptionString, args) {
    ...
    function calculateMatchingScore(assertionString) {
        try {
            that.lookupAssertionRule(subject, assertionString, args);
            // Give some points for matching and for being specific
            return 5 + Math.max.apply(null, that.assertions[assertionString].map(function (assertion) {
...
Unexpected.prototype.lookupAssertionRule = function (subject, testDescriptionString, args) {
    ...
    if (machingAssertions.length > 1) {
        ...
        if (machingAssertions[0].specificity > machingAssertions[1].specificity) {
            return machingAssertions[0];
        }

        var ambiguousAssertions = machingAssertions.sort(function (a, b) {
            return b.specificity - a.specificity;
        }).filter(function (assertionWithScore, i, arr) {
            return assertionWithScore.specificity === arr[0].specificity;
        }).map(function (assertion) {
            return assertion.declaration;
        });

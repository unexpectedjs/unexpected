...
var anyType = {
    ...
    diff: function (actual, expected, output, diff, inspect) {
        var actualInspected = inspect(output.clone(), actual).removeFormatting();
        var expectedInspected = inspect(output.clone(), expected).removeFormatting();
        var result = {
            conflict: false,
            text: output.clone(),
            annotation: output.clone()
        };
        // todo output with error style
        if (this.equal(actual, expected)) {
            result.text.append(actualInspected);
        } else if (typeof expected === 'undefined') {
            result.conflict = true;
            result.text.append(actualInspected);
            result.annotation.text('should be removed');
        } else if (typeof actual === 'undefined') {
            result.conflict = true;
            result.text.append(expectedInspected);
            result.annotation.text('is missing');
        } else {
            result.conflict = true;
            result.text.append(actualInspected);
            result.annotation.text('should be ').block(expectedInspected);
        }
        return result;
    },

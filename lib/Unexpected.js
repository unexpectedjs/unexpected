...
function Unexpected(options) {
  ...
  this.findTypeOf = obj =>
    utils.findFirst(that.types || [], type => type.identify && type.identify(obj)) ||
    (that.parent && that.parent.findTypeOf(obj));
  this.findTypeOfWithParentType = (obj, requiredParentType) =>
    utils.findFirst(
      that.types || [],
      type =>
        type.identify && type.identify(obj) && (!requiredParentType || type.is(requiredParentType))
    ) ||
      ...
      makePromise.settle(promises).then(() => {
        ...
        if (
          !groupEvaluations.some(groupEvaluation =>
            groupEvaluation.every(evaluation => evaluation.promise.isFulfilled())
          )
        ) {
...
Unexpected.prototype.parseAssertion = function(assertionString) {
  ...
  if (
    [assertion.subject]
      ...
      .some(argRequirements =>
        argRequirements.some(argRequirement => argRequirement.type.is("assertion"))
      )
  ) {
  ...
  var assertionRequirements = lastArgRequirements.filter(argRequirement =>
    argRequirement.type.is("assertion")
  );
  ...
  if (assertionRequirements.some(argRequirement => argRequirement.maximum !== 1)) {
      ...
      expandedAssertions.forEach(expandedAssertion => {
        ...
        maxNumberOfArgs = Math.max(
          ...
          assertionDeclaration.args.reduce(
            (previous, argDefinition) =>
              previous + (argDefinition.maximum === null ? Infinity : argDefinition.maximum),
            0
          )
        );
...
Unexpected.prototype.throwAssertionNotFoundError = function(subject, testDescriptionString, args) {
  ...
  var bestMatch = assertionsWithScore
    ...
    .slice(0, 10)
    .filter(
      (assertionsWithScore, i, arr) => Math.abs(assertionsWithScore.score - arr[0].score) <= 2
    )
    .sort((a, b) => {
    ...
    if (relaxed) {
      if (assertionType.identify === false) {
        return that.types.some(
          type => type.identify && type.is(assertionType) && type.identify(value)
        );
      }
    ...
    wrappedExpect.argsOutput = args.map((arg, i) => {
      ...
      if (
        ...
        ((argRule && argRule.type.is("assertion")) ||
          wrappedExpect._getAssertionIndices().includes(i))
      ) {

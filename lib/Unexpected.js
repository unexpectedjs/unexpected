...
var runningPromises = require('./runningPromises');
...
function setupHooks() {
    if (typeof afterEach === 'function' && !afterEach.hooked) {
        afterEach.hooked = true;
        afterEach(function (done) {
            if (runningPromises.hasPendingPromises()) {
                var title = this.currentTest.title;
                Promise.all(runningPromises.promises).caught(function () {
                    // Ignore
                }).lastly(function () {
                    runningPromises.clear();
                    done(new Error(title));
                });
            } else {
                done();
            }

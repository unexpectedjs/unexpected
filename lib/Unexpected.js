...
function expandTypeAlternations(assertion) {
...
Unexpected.prototype.parseAssertion = function (assertionString) {
    ...
    if (assertion.args.slice(0, -1).some(function (type) { return hasVarargs(type); })) {
        throw new SyntaxError('Only the last argument type can have varargs: ' + assertionString);
    }

    return expandTypeAlternations(assertion);
};
...
Unexpected.prototype.lookupAssertionRule = function (subject, testDescriptionString, args) {
    ...
    function matchesType(type, value, key) {
        if (type.identify === false) {
            return findTypeOf(value, key).is(type);
        } else {
            return type.identify(value) && findTypeOf(value, key).is(type);
        }
    }
    ...
    function matchesAssertion(assertion) {
        if (!matchesType(assertion.subject.type, subject, 'subject')) {
            return false;
        }
        ...
        return args.every(function (arg, i) {
            if (i < assertion.args.length - 1) {
                return matchesType(assertion.args[i].type, arg, i);
            } else {
                var lastRequirement = assertion.args[assertion.args.length - 1];
                if (
                    i - assertion.args.length < lastRequirement.minimum &&
                        lastRequirement.maximum < i - assertion.args.length
                ) {
                    return false;
                }
                return matchesType(lastRequirement.type, arg, i);
            }
        });
    }

...
function Unexpected(options) {
  ...
  var that = this;
  this.getType = typeName => that.typeByName[typeName] || (that.parent && that.parent.getType(typeName));
  this.findTypeOf = obj => utils.findFirst(that.types || [], type => type.identify && type.identify(obj)) ||
  (that.parent && that.parent.findTypeOf(obj));
  this.findTypeOfWithParentType = (obj, requiredParentType) => utils.findFirst(that.types || [], type => type.identify &&
  type.identify(obj) &&
...
function writeGroupEvaluationsToOutput(output, groupEvaluations) {
  ...
  var hasAndClauses = groupEvaluations.some(groupEvaluation => groupEvaluation.length > 1);
    ...
    groupEvaluation.forEach((evaluation, j) => {
      ...
      if (isRejected && !groupFailed) {
        ...
        output.block(output => {
          ...
          var subjectOutput = output => {
            output.appendInspected(subject);
          };
          var args = expectation.slice(2);
          var argsOutput = args.map(arg => output => {
    ...
    return oathbreaker(
      makePromise.settle(promises).then(() => {
        groupEvaluations.forEach(groupEvaluation => {
          groupEvaluation.forEach(evaluation => {
            if (
              evaluation.promise.isRejected() &&
              evaluation.promise.reason().errorMode === "bubbleThrough"
            ) {
        ...
        if (
          !groupEvaluations.some(groupEvaluation => groupEvaluation.every(evaluation => evaluation.promise.isFulfilled()))
        ) {
...
Unexpected.prototype.equal = function(actual, expected, depth, seen) {
  ...
  return this.findCommonType(actual, expected).equal(actual, expected, (a, b) => that.equal(a, b, depth - 1, seen));
};

Unexpected.prototype.inspect = function(obj, depth, outputOrFormat) {
  var seen = [];
  var that = this;
  var printOutput = (obj, currentDepth, output) => {
  ...
  var output =
    typeof outputOrFormat === "string" ? this.createOutput(outputOrFormat) : outputOrFormat;
...
Unexpected.prototype.parseAssertion = function(assertionString) {
  ...
  function hasVarargs(types) {
    return types.some(type => type.minimum !== 1 || type.maximum !== 1);
  }
  ...
  if (
    assertion.args.some(arg => typeof arg === "string")
  ) {
  ...
  if (
    [assertion.subject].concat(assertion.args.slice(0, -1)).some(argRequirements => argRequirements.some(argRequirement => argRequirement.type.is("assertion")))
  ) {
  ...
  var assertionRequirements = lastArgRequirements.filter(argRequirement => argRequirement.type.is("assertion"));
  ...
  if (
    assertionRequirements.some(argRequirement => argRequirement.maximum !== 1)
  ) {
...
function calculateAssertionSpecificity(assertion) {
  return [assertion.subject.type.level].concat(
    assertion.args.map(argRequirement => {
...
Unexpected.prototype.addAssertion = function(patternOrPatterns, handler, childUnexpected) {
  ...
  patterns.forEach(pattern => {
    ...
    assertionDeclarations.forEach(assertionDeclaration => {
      ...
      expandedAssertions.forEach(expandedAssertion => {
        Object.keys(expandedAssertion.flags).forEach(flag => {
          defaultValueByFlag[flag] = false;
        });
        maxNumberOfArgs = Math.max(
          maxNumberOfArgs,
          assertionDeclaration.args.reduce((previous, argDefinition) => previous + (argDefinition.maximum === null ? Infinity : argDefinition.maximum), 0)
        );
  ...
  if (handler.length - 2 > maxNumberOfArgs) {
    throw new Error(
      "The provided assertion handler takes " +
        (handler.length - 2) +
        " parameters, but the type signature specifies a maximum of " +
        maxNumberOfArgs +
        ":\n\n    " +
  ...
  assertionHandlers.forEach(handler => {
    ...
    if (!assertionHandlers) {
      ...
      while (
        ...
        compareSpecificities(handler.specificity, assertionHandlers[i].specificity) > 0
      ) {
        i += 1;
      }
...
Unexpected.prototype.addType = function(type, childUnexpected) {
  ...
  extendedBaseType.inspect = (value, depth, output) => {
    ...
    return baseType.inspect(value, depth, output, (value, depth) => output.clone().appendInspected(value, depth));
  };
  ...
  extendedBaseType.equal = (actual, expected) => baseType.equal(actual, expected, that.equal.bind(that));
...
Unexpected.prototype.use = function(plugin) {
  if (
    (typeof plugin !== "function" &&
      (typeof plugin !== "object" || typeof plugin.installInto !== "function")) ||
  ...
  var existingPlugin = utils.findFirst(this.installedPlugins, installedPlugin => {
  ...
  if (existingPlugin) {
    if (
      ...
      (typeof plugin.version !== "undefined" && plugin.version === existingPlugin.version)
    ) {
      // No-op
      return this.expect;
    } else {
...
Unexpected.prototype.withError = (body, handler) => oathbreaker(
...
Unexpected.prototype.throwAssertionNotFoundError = function(subject, testDescriptionString, args) {
    ...
    this.fail({
      message: function(output) {
        ...
        var assertionDeclarations = Object.keys(
          candidateHandlers.reduce((result, handler) => {
  ...
  var bestMatch = assertionsWithScore
    ...
    .slice(0, 10)
    .filter((assertionsWithScore, i, arr) => Math.abs(assertionsWithScore.score - arr[0].score) <= 2)
    .sort((a, b) => {
  ...
  function matches(value, assertionType, key, relaxed) {
    ...
    if (relaxed) {
      if (assertionType.identify === false) {
        return that.types.some(type => type.identify && type.is(assertionType) && type.identify(value));
      }
...
Unexpected.prototype._expect = function expect(context, args) {
  ...
  function executeExpect(context, subject, testDescriptionString, args) {
    ...
    var wrappedExpect = function(subject, testDescriptionString) {
      ...
      return wrappedExpect.callInNestedContext(() => executeExpect(context.child(), subject, testDescriptionString, args));
    };
  ...
  function asyncMisusage(message) {
    ...
    that.expect.fail(output => {
      output
        ...
        .indentLines()
        .i()
        .text("return expect('test.txt', 'to have content', 'Content read asynchroniously');")
        .nl()
        .outdentLines()
        .text("});");
    });
...
Unexpected.prototype.diff = function(a, b, output, recursions, seen) {
  ...
  return makeDiffResultBackwardsCompatible(
    this.findCommonType(a, b).diff(
      ...
      b,
      output,
      (actual, expected) => that.diff(actual, expected, output.clone(), recursions - 1, seen),
...
Unexpected.prototype.toString = function() {
  ...
  var declarations = [];
  var pen = magicpen();
  Object.keys(assertions).sort().forEach(key => {
    assertions[key].forEach(assertion => {
  ...
  function createPermutations(tokens, index) {
    ...
    if (isFlag(token)) {
      ...
      return token
        ...
        .split(/\|/)
        .reduce((result, alternation) => result.concat(
        tail.map(pattern => ({

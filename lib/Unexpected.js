...
Unexpected.prototype.equal = function (actual, expected, depth, seen) {
    ...
    var matchingType = this.findTypeOf(actual, expected);
...
Unexpected.prototype.inspect = function (obj, depth) {
    ...
    var seen = [];
    var that = this;
    var printOutput = function (output, obj, currentDepth) {
        ...
        var matchingType = that.findTypeOf(obj);

        if (matchingType) {
            return matchingType.inspect(output, obj, function (v, childDepth) {
                seen.push(obj);
                if (typeof childDepth === 'undefined') {
                    childDepth = currentDepth - 1;
                }
                return printOutput(output.clone(), v, childDepth);
            }, currentDepth);
        } else {
            return output;
        }
...
function installExpectMethods(unexpected, expectFunction) {
    ...
    expect.findTypeOf = bind(unexpected.findTypeOf, unexpected);
...
Unexpected.prototype.diff = function (a, b) {
    var output = this.output.clone();
    var that = this;
    var matchingCustomType = this.findTypeOf(a, b);

    if (matchingCustomType) {
        return matchingCustomType.diff(output, a, b, function (actual, expected) {

...
Unexpected.prototype.parseAssertion = function(assertionString) {
  ...
  if (
    assertionRequirements.some(({maximum}) => maximum !== 1)
  ) {
    throw new SyntaxError(
...
Unexpected.prototype.throwAssertionNotFoundError = function(
  ...
  const bestMatch = assertionsWithScore
    .sort(({score, assertion}, {score, assertion}) => {
      const c = score - score;
      if (c !== 0) {
        return c;
      }
    ...
    .sort(({assertion, score}, {assertion, score}) => {
      const c = compareAssertions(assertion, assertion);
      if (c !== 0) {
        return c;
      }

...
var anyType = {
    ...
    diff: function (actual, expected, output, diff, inspect) {
        var actualInspected = inspect(actual);
        var expectedInspected = inspect(expected);
        var result = {
            conflict: false,
            text: output.clone(),
        ...
        if (this.equal(actual, expected)) {
            ...
            result.text.append(actualInspected);
            result.annotation.error('should be removed');
        } else {
            result.conflict = true;
            result.text.append(actualInspected);
            result.annotation.error('should be ').block(expectedInspected);
        }
        return result;
    },
...
Unexpected.prototype.setErrorMessage = function (err) {
    ...
    var message = err.output;
    if (err.diff) {
        message.nl(2).blue('Diff:').nl(2).append(err.diff);
    }
    err.message = message.toString(outputFormat);
};

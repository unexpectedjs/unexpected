...
Unexpected.prototype.cast = function (value, typeOrTypeName) {
    var type;
    if (typeof type === 'string') {
        type = this.getType(typeOrTypeName);
    } else {
        type = typeOrTypeName;
    }
    if (!type) {
        throw new Error('No such type: ' + typeOrTypeName);
    }
    var valueType = this.findTypeOf(value);
    if (!valueType.is(type)) {
        throw new Error('Cannot cast ' + valueType.name + ' to ' + type.name);
    }
    return {
        _expectCast: true,
        type: type,
...
function installExpectMethods(unexpected, expectFunction) {
    ...
    expect.cast = unexpected.cast.bind(unexpected);
...
Unexpected.prototype.expect = function expect(subject, testDescriptionString) {
    ...
    function executeExpect(subject, testDescriptionString, args) {
        ...
        var matchingType;
        var unwrappedSubject;
        if (subject && subject._expectCast) {
            matchingType = subject.type;
            unwrappedSubject = subject.value;
        } else {
            matchingType = that.findTypeOf(subject);
            unwrappedSubject = subject;
        }
        ...
        if (assertionRule) {
            ...
            wrappedExpect.cast = that.cast.bind(that);
            ...
            var result = handler.apply(assertion, [wrappedExpect, unwrappedSubject].concat(args.map(function (arg) {
                return arg && arg._expectCast ? arg.value : arg;
            })));

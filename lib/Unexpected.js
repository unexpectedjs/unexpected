...
function createExpectIt(expect, expectations) {
    ...
    function expectIt(subject) {
        ...
        return oathbreaker(
            expect.promise.settle(promises).then(function() {
                groupEvaluations.forEach(function(groupEvaluation) {
                    groupEvaluation.forEach(function(evaluation) {
                        if (
                            ...
                            evaluation.promise.reason().errorMode ===
                                'bubbleThrough'
                        ) {
...
Unexpected.prototype.equal = function(actual, expected, depth, seen) {
    ...
    return this.findCommonType(
        actual,
        expected
    ).equal(actual, expected, function(a, b) {
...
Unexpected.prototype.inspect = function(obj, depth, outputOrFormat) {
    ...
    var printOutput = function(obj, currentDepth, output) {
        ...
        return objType.inspect(
            obj,
            currentDepth,
            output,
            function(v, childDepth) {
                output = output.clone();
                seen.push(obj);
    ...
    var output = typeof outputOrFormat === 'string'
    ...
    function createPermutations(args, i) {
        ...
        args[i].forEach(function(arg) {
            ...
            if (tails.length) {
                ...
                result.push([
                    {
                        type: that.typeByName['expect.it'],
                        minimum: 1,
                        maximum: 1
                    }
                ]);
...
Unexpected.prototype.parseAssertion = function(assertionString) {
    ...
    function lookupType(typeName) {
        ...
        if (!result) {
            throw new Error(
                'Unknown type: ' + typeName + ' in ' + assertionString
            );
        }
        return result;
    }
    ...
    function parseType(assertionString) {
        return assertionString.split('|').map(function(type) {
            var matchNameAndOperator = type.match(
    ...
    assertionString.replace(
        /\s*<((?:[a-z_](?:|[a-z0-9_.-]*[_a-z0-9])[?*+]?)(?:\|(?:[a-z_](?:|[a-z0-9_.-]*[_a-z0-9])[?*+]?))*)>|\s*([^<]+)/ig,
        function($0, $1, $2, index) {
            if (index !== nextIndex) {
                throw new SyntaxError(
                    'Cannot parse token at index ' +
                        nextIndex +
                        ' in ' +
                        assertionString
                );
    ...
    if (
        assertion.args.some(function(arg) {
    ...
    var assertionRequirements = lastArgRequirements.filter(
        function(argRequirement) {
            return argRequirement.type.is('assertion');
        }
    );
    ...
    if (assertionRequirements.length > 0 && lastArgRequirements.length > 1) {
        throw new SyntaxError(
            '<assertion> cannot be alternated with other types: ' +
                assertionString
        );
    }

    if (
        assertionRequirements.some(function(argRequirement) {
...
function calculateAssertionSpecificity(assertion) {
    return [assertion.subject.type.level].concat(
        assertion.args.map(function(argRequirement) {
            var bonus = argRequirement.minimum === 1 &&
                argRequirement.maximum === 1
                ? 0.5
                : 0;
...
Unexpected.prototype.addAssertion = function(patternOrPatterns, handler) {
    ...
    patterns.forEach(function(pattern) {
        ...
        assertionDeclarations.forEach(function(assertionDeclaration) {
            ensureValidUseOfParenthesesOrBrackets(
                assertionDeclaration.assertion
            );
            var expandedAssertions = expandAssertion(
                assertionDeclaration.assertion
            );
            expandedAssertions.forEach(function(expandedAssertion) {
                ...
                maxNumberOfArgs = Math.max(
                    maxNumberOfArgs,
                    assertionDeclaration.args.reduce(
                        function(previous, argDefinition) {
                            return previous +
                                (argDefinition.maximum === null
                                    ? Infinity
                                    : argDefinition.maximum);
                        },
                        0
                    )
                );
...
Unexpected.prototype.addType = function(type) {
    ...
    if (this.getType(type.name)) {
        throw new Error(
            'The type with the name ' + type.name + ' already exists'
        );
    }
...
Unexpected.prototype.use = function(plugin) {
    if (
        ...
        (typeof plugin.dependencies !== 'undefined' &&
    ...
    var existingPlugin = utils.findFirst(
        ...
        function(installedPlugin) {
            if (installedPlugin === plugin) {
                ...
                return pluginName &&
                    pluginName === getPluginName(installedPlugin);
            }
        }
    );
    ...
    if (plugin.dependencies) {
        ...
        var unfulfilledDependencies = plugin.dependencies.filter(
            function(dependency) {
        ...
        if (unfulfilledDependencies.length === 1) {
            throw new Error(
                pluginName + ' requires plugin ' + unfulfilledDependencies[0]
            );
            ...
            throw new Error(
                pluginName +
                    ...
                    ' and ' +
                    unfulfilledDependencies[unfulfilledDependencies.length - 1]
            );
        }
    }
...
Unexpected.prototype.throwAssertionNotFoundError = function(
    ...
    function compareAssertions(a, b) {
        ...
        return compareSpecificities(
            aAssertion.specificity,
            bAssertion.specificity
        );
    }

    assertionStrings.forEach(
        function(assertionString) {
    ...
    var bestMatch = assertionsWithScore
        ...
        .slice(0, 10)
        .filter(function(assertionsWithScore, i, arr) {
...
Unexpected.prototype.expect = function expect(subject, testDescriptionString) {
    ...
    function executeExpect(subject, testDescriptionString, args) {
        var assertionRule = that.lookupAssertionRule(
            subject,
            testDescriptionString,
            args
        );
        ...
        if (!assertionRule) {
            ...
            for (var n = tokens.length - 1; n > 0; n -= 1) {
                ...
                var argsWithAssertionPrepended = [
                    tokens.slice(n).join(' ')
                ].concat(args);
            ...
            if (!assertionRule) {
                that.throwAssertionNotFoundError(
                    subject,
                    testDescriptionString,
                    args
                );
        ...
        var wrappedExpect = function(subject, testDescriptionString) {
            if (arguments.length === 0) {
                ...
                testDescriptionString && testDescriptionString._expectIt
            ) {
        ...
        return oathbreaker(
            assertionRule.handler.apply(
                wrappedExpect,
                [wrappedExpect, subject].concat(args)
            )
        );
    }
...
Unexpected.prototype.clone = function() {
    ...
    Object.keys(this.assertions).forEach(
        function(assertion) {
                        ...
                        return result.concat(
                            tail.map(function(pattern) {
                                return {
                                    ...
                                    text: alternation
                                        ? alternation + pattern.text
                                        : pattern.text.replace(/^ /, ''),
                                    flags: pattern.flags,
                                    alternations: [alternation].concat(
...
function ensureValidUseOfParenthesesOrBrackets(pattern) {
    ...
    for (var i = 0; i < pattern.length; i += 1) {
        ...
        if (c === ']' && counts['['] >= counts[']']) {
            if (counts['['] === counts[']'] + 1) {
                throw new Error(
                    "Assertion patterns must not contain flags with brackets: '" +
                        pattern +
                        "'"
                );
            }
            ...
            if (counts['('] !== counts[')']) {
                throw new Error(
                    "Assertion patterns must not contain flags with parentheses: '" +
                        pattern +
                        "'"
                );
            }
            ...
            if (pattern.charAt(i - 1) === '[') {
                throw new Error(
                    "Assertion patterns must not contain empty flags: '" +
                        pattern +
                        "'"
                );
            }
            ...
            if (counts['('] === counts[')'] + 1) {
                throw new Error(
                    "Assertion patterns must not contain alternations with parentheses: '" +
                        pattern +
                        "'"
                );
            }
            ...
            if (counts['['] !== counts[']']) {
                throw new Error(
                    "Assertion patterns must not contain alternations with brackets: '" +
                        pattern +
                        "'"
                );
            }
    ...
    if (counts['['] !== counts[']']) {
        throw new Error(
            "Assertion patterns must not contain unbalanced brackets: '" +
                pattern +
                "'"
        );
    }
    ...
    if (counts['('] !== counts[')']) {
        throw new Error(
            "Assertion patterns must not contain unbalanced parentheses: '" +
                pattern +
                "'"
        );
    }

    ...
    assertion = {
      ...
      args: tokens.slice(2).map(function(token) {
        if (typeof token === 'string') {
          return [
            {
              type: that.typeByName.assertion,
              minimum: 1,
              maximum: 1,
              variants: expandAssertion(token).reduce(function(
                variants,
                expandedAssertion
              ) {
  ...
  if (
    [assertion.subject]
      ...
      .some(argRequirements =>
        argRequirements.some(
          ({ type, variants }) => type.is('assertion') && !variants
        )
      )
  ) {
    throw new SyntaxError(
    ...
    if (relaxed) {
      if (arg.type.identify === false) {
        return that.types.some(
          type => type.identify && type.is(arg.type) && type.identify(value)
        );
      }
...
Unexpected.prototype._expect = function expect(context, args) {
  ...
  function executeExpect(context, subject, testDescriptionString, args) {
    ...
    if (
      assertionRule.args.some(function(arg) {
        return arg.variants;
      })
    ) {
      args = wrappedExpect.args.filter(function(arg, i) {
        if (i < assertionRule.args.length && assertionRule.args[i].variants) {
          ...
          wrappedExpect.alternations = wrappedExpect.alternations.concat(
            assertionRule.args[i].variants[args[i]].alternations
          );
        } else {
          return true;
        }
    ...
    wrappedExpect.argsOutput = wrappedExpect.args.map((arg, i) => {

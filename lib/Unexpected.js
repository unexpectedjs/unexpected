...
function getNextTypeIndex(expect) {
    if (expect.types.length) {
        return expect.types[0].index + 1;
    } else {
        return getNextTypeIndex(expect.parent);
    }
}

function getNextAbstractTypeIndex(expect) {
    if (expect.types.length) {
        return expect.types[expect.types.length - 1].index - 1;
    } else {
        return getNextAbstractTypeIndex(expect.parent);
    }
}
...
Unexpected.prototype.addType = function (type, childUnexpected) {
    ...
    if (extendedType.identify === false) {
        extendedType.index = getNextAbstractTypeIndex(this);
        this.types.push(extendedType);
    } else {
        extendedType.index = getNextTypeIndex(this);

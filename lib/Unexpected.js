...
function createExpect(options = {}) {
  ...
  expect.hook = fn => {
    expect._expect = fn(expect._expect.bind(expect));
  };

  expect.assertions = options.assertions || {};
  expect.typeByName = options.typeByName || { any: anyType };
  expect.types = options.types || [anyType];
  if (options.output) {
    expect.output = options.output;
  } else {
    expect.output = magicpen();
    expect.output.inline = false;
    expect.output.diff = false;
  }
  expect._outputFormat = options.format || magicpen.defaultFormat;
  expect.installedPlugins = options.installedPlugins || [];

  expect.getType = typeName =>
    expect.typeByName[typeName] ||
    (expect.parent && expect.parent.getType(typeName));
  expect.findTypeOf = obj =>
    utils.findFirst(
      expect.types || [],
      type => type.identify && type.identify(obj)
    ) ||
    (expect.parent && expect.parent.findTypeOf(obj));
  expect.findTypeOfWithParentType = (obj, requiredParentType) =>
    utils.findFirst(
      expect.types || [],
      type =>
        type.identify &&
    ...
    (expect.parent &&
      expect.parent.findTypeOfWithParentType(obj, requiredParentType));
  expect.findCommonType = function(a, b) {
  ...
  expect._wrappedExpectProto = createWrappedExpectProto(expect);
          ...
          error.createDiff = (output, diff, inspect, equal) => {
            ...
            return value(
              ...
              function diff(actual, expected) {
                return this.expect.diff(actual, expected, childOutput.clone());
              },
...
expectPrototype.addType = function(type, childUnexpected) {
  if (this._frozen) {
    throw new Error(
...
expectPrototype._expect = function expect(context, args) {
  ...
  function executeExpect(context, subject, testDescriptionString, args) {
    ...
    if (
      ...
      // FIXME: Should be renamed to assertionRule.expect
      return assertionRule.unexpected(subject, testDescriptionString, ...args);
    }
  ...
  try {
    ...
    if (utils.isPromise(result)) {
      ...
      if (result.isPending()) {
        that.notifyPendingPromise(result);
    ...
    return addAdditionalPromiseMethods(result, that, subject);
  } catch (e) {
    if (e && e._isUnexpected) {
      let newError = e;
...
expectPrototype.child = function() {
  const childExpect = createExpect({
    assertions: {},
    types: [],
    typeByName: {},
    output: this.output.clone(),

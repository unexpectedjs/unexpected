...
const isPendingPromise = require("./isPendingPromise");
...
Unexpected.prototype.parseAssertion = function(assertionString) {
  ...
  const assertionRequirements = lastArgRequirements.filter(({type}) => type.is("assertion")
  );
    ...
    assertionDeclarations.forEach(({assertion, args, subject}) => {
      ...
      expandedAssertions.forEach(({flags, alternations, text}) => {
        ...
        maxNumberOfArgs = Math.max(
          maxNumberOfArgs,
          args.reduce(
            (previous, {maximum}) => previous + (maximum === null ? Infinity : maximum),
            0
          )
        );
...
Unexpected.prototype.use = function(plugin) {
  ...
  const existingPlugin = utils.findFirst(this.installedPlugins, installedPlugin => {
...
Unexpected.prototype.throwAssertionNotFoundError = function(subject, testDescriptionString, args) {
  ...
  const bestMatch = assertionsWithScore
    ...
    .slice(0, 10)
    .filter(
      ({score}, i, arr) => Math.abs(score - arr[0].score) <= 2
    )
    .sort((a, b) => {
...
Unexpected.prototype._expect = function expect(context, args) {
  ...
  function executeExpect(context, subject, testDescriptionString, args) {
    let assertionRule = that.lookupAssertionRule(subject, testDescriptionString, args);
    ...
    if (!assertionRule) {
      ...
      OUTER: for (let n = tokens.length - 1; n > 0; n -= 1) {
        ...
        const argsWithAssertionPrepended = [remainingTokens.join(" ")].concat(args);
...
Unexpected.prototype.diff = function(a, b, output = this.createOutput(), recursions, seen) {
  const that = this;

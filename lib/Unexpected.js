...
function appendAssertionHandler(assertions, testDescriptionString, assertionHandler) {
    var assertionHandlers = assertions[testDescriptionString];
    if (!assertionHandlers) {
        assertions[testDescriptionString] = [assertionHandler];
    } else {
        // TODO ensure we don't add duplicates
        assertionHandlers.push(assertionHandler);
    }
}
...
Unexpected.prototype.addAssertion = function (types, patterns, handler) {
    ...
    patterns.forEach(function (pattern) {
        ...
        expandedAssertions.forEach(function (expandedAssertion) {
            ...
            appendAssertionHandler(assertions, expandedAssertion.text, {
                handler: handler,
                flags: extend({}, defaultValueByFlag, expandedAssertion.flags),
...
Unexpected.prototype.lookupAssertionRule = function (subject, testDescriptionString, args) {
    ...
    function matchesType(type, value) {
        if (type.identify === false) {
            // TODO slow
            return that.findTypeOf(value).is(type);
        } else {
            return type.identify(value);
        }
    }

    var machingAssertions = (this.assertions[testDescriptionString] || []).filter(function (assertion) {
        var matchingSubject = assertion.subject.some(function (subjectRequirement) {
            return matchesType(subjectRequirement.type, subject);
        });
    ...
    if (machingAssertions.length === 0) {
        // TODO no matching assertion
        throw new Error('No maching assertion');
    }

    function scoreForRequirements(requireArguments) {
    ...
    if (machingAssertions.length > 1) {
        ...
        // TODO can be optimized by only trying to find the best match
        var machingAssertionsWithScore = machingAssertions.map(function (assertion) {
            var score = scoreForRequirements(assertion.subject);
            score += assertion.args.reduce(function (result, requirements) {
                return result + scoreForRequirements(requirements);
            }, 0);
            return { score: score, assertion: assertion };
        });
        machingAssertionsWithScore.sort(function (a, b) {
            return b.score - a.score;
        });
        if (machingAssertionsWithScore[0].score === machingAssertionsWithScore[1].score) {
            var ambiguousAssertions = machingAssertionsWithScore.filter(function (assertionWithScore) {
                return assertionWithScore.score === machingAssertionsWithScore[0].score;
            }).map(function (assertionWithScore) {
                return assertionWithScore.assertion.declaration;
            });
            that.fail({
                errorMode: 'bubbleThough',
            ...
            throw new Error('Ambiguous assertions: ');
        }
        return machingAssertionsWithScore[0].assertion;
    }
};

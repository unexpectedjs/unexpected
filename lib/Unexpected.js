                ...
                maxNumberOfArgs = Math.max(
                    ...
                    assertionDeclaration.args.reduce(
                        function(previous, argDefinition) {
                            return previous +
                                (argDefinition.maximum === null ? Infinity : argDefinition.maximum);
                        },
                        0
                    )
                );
...
Unexpected.prototype.expect = function expect(subject, testDescriptionString) {
    ...
    function executeExpect(subject, testDescriptionString, args) {
        ...
        return oathbreaker(
            assertionRule.handler.apply(wrappedExpect, [wrappedExpect, subject].concat(args))
        );
    }
...
var expandAssertion = (function() {
    ...
    function createPermutations(tokens, index) {
        ...
        if (isFlag(token)) {
            ...
            return token.substr(1, token.length - 2).split(/\|/).reduce(function(
                // Remove parentheses
                result,
                alternation
            ) {

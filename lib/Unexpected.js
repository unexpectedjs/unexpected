...
Unexpected.prototype.parseAssertion = function (assertionString) {
    ...
    // TODO this can be cache on the unexpected instanced
    var typesByName = this.types.reduce(function (result, type) {
        result[type.name] = type;
        return result;
    }, {});

    function lookupType(typeName) {
        var result = typesByName[typeName];
        if (!result) {
    ...
    function parseType(assertionString) {
        return assertionString.split('|').map(function (typeDeclaration) {
            var varargs = /^\.\.\./.test(typeDeclaration);
            if (varargs) {
                return { varargs: lookupType(typeDeclaration.substr(3)) };
            } else {
                return lookupType(typeDeclaration);
            }
        });
    }
    ...
    assertionString.replace(/\s*<((?:(?:\.\.\.)?[a-z_](?:|[a-z0-9_.-]*[_a-z0-9]))(?:|(?:(?:\.\.\.)?[a-z_](?:\|[a-z0-9_.-]*[_a-z0-9])))?)>|\s*([^<]+)/g, function ($0, $1, $2, index) {
        if (index !== nextIndex) {

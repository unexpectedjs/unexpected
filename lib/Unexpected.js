...
function writeGroupEvaluationsToOutput(output, groupEvaluations) {
  ...
  var hasAndClauses = groupEvaluations.some(
    groupEvaluation => groupEvaluation.length > 1
  );
      ...
      makePromise.settle(promises).then(() => {
        ...
        if (
          !groupEvaluations.some(groupEvaluation =>
            groupEvaluation.every(evaluation =>
              evaluation.promise.isFulfilled()
            )
          )
        ) {
  ...
  if (
    [assertion.subject]
      ...
      .some(argRequirements =>
        argRequirements.some(argRequirement =>
...
function calculateAssertionSpecificity(assertion) {
  return [assertion.subject.type.level].concat(
    assertion.args.map(argRequirement => {
      var bonus =
        argRequirement.minimum === 1 && argRequirement.maximum === 1 ? 0.5 : 0;
...
Unexpected.prototype.throwAssertionNotFoundError = function(
  ...
  var bestMatch = assertionsWithScore
    ...
    .slice(0, 10)
    .filter(
      (assertionsWithScore, i, arr) =>
        Math.abs(assertionsWithScore.score - arr[0].score) <= 2
    )
    .sort((a, b) => {

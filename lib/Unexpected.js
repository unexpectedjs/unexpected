...
function writeGroupEvaluationsToOutput(output, groupEvaluations) {
  ...
  const hasAndClauses = groupEvaluations.some(
...
Unexpected.prototype.parseAssertion = function(assertionString) {
  ...
  const assertionRequirements = lastArgRequirements.filter(argRequirement =>
    argRequirement.type.is('assertion')
  );
...
Unexpected.prototype.addAssertion = function(
  ...
  patterns.forEach(pattern => {
    ...
    assertionDeclarations.forEach(assertionDeclaration => {
      ...
      const expandedAssertions = expandAssertion(assertionDeclaration.assertion);
...
Unexpected.prototype.addType = function(type, childUnexpected) {
  ...
  const extendedType = extend({}, baseType, type, { baseType: extendedBaseType });

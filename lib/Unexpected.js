...
Unexpected.prototype.lookupAssertionRule = function (subject, testDescriptionString, args) {
    ...
    function matches(value, valueType, type) {
    ...
    function matchesHandler(handler) {
        if (!matches(subject, subjectType, handler.subject.type)) {
            return false;
        }
        ...
        return args.every(function (arg, i) {
            ...
            if (i < handler.args.length - 1) {
                return matches(arg, argType, handler.args[i].type);
            } else {
                return matches(arg, argType, lastRequirement.type);
            }
        });
    }

...
Unexpected.prototype.expandTypeAlternations = function (assertion) {
    ...
    function createPermutations(args, i) {
        ...
        args[i].forEach(function (arg) {
            ...
            } else if (arg.type === assertionType) {
                result.push([
                    { type: that.typeByName['string'], minimum: 1, maximum: 1 },
...
Unexpected.prototype.parseAssertion = function (assertionString) {
    ...
    if ([assertion.subject].concat(assertion.args.slice(0, -1)).some(function (argRequirements) {
        return argRequirements.some(function (argRequirement) {
            return argRequirement.type === assertionType;
        });
    })) {
    ...
    if (assertionRequirements.length > 0 && lastArgRequirements.length > 1) {
        throw new SyntaxError('<assertion> can be alternated with other types: ' + assertionString);
    }

    if (assertionRequirements.some(function (argRequirement) {
        return argRequirement.minimum !== 1 || argRequirement.maximum !== 1;
    })) {
        throw new SyntaxError('<assertion> cannot have varargs: ' + assertionString);
    }
...
function calculateAssertionSpecificity(assertion) {
    var factor = 0.01;
    return assertion.args.reduce(function (result, argRequirement) {
        var bonus = argRequirement.minimum === 1 && argRequirement.maximum === 1 ?
            0.5 : 0;
        result = result + factor * (bonus + argRequirement.type.level);
        factor *= factor;
        return result;
    }, assertion.subject.type.level);
}
...
Unexpected.prototype.addAssertion = function (types, patterns, handler) {
    ...
    if (typeof patterns === 'function') {
        handler = patterns;
        patterns = Array.isArray(types) ? types : [types];
    ...
    assertionHandlers.forEach(function (handler) {
        ...
        if (!assertionHandlers) {
            ...
            var i = 0;
            while (i < assertionHandlers.length && handler.specificity < assertionHandlers[i].specificity) {
                i += 1;
            }
...
Unexpected.prototype.throwAssertionNotFoundError = function (subject, testDescriptionString, args) {
    ...
    function calculateMatchingScore(assertionString) {
        try {
...
Unexpected.prototype.lookupAssertionRule = function (subject, testDescriptionString, args) {
    ...
    var subjectType = that.findTypeOf(subject);
    var argTypes = new Array(args.length);
    for (var i = 0; i < args.length; i += 1) {
        argTypes[i] = that.findTypeOf(args[i]);
    }

    function matchesAssertion(assertion) {
        if (!subjectType.is(assertion.subject.type)) {
            return false;
        }

        var requireArgumentsLength = calculateLimits(assertion.args);
        ...
        var lastRequirement = assertion.args[assertion.args.length - 1];
        return args.every(function (arg, i) {
            var argType = argTypes[i];
            if (i < assertion.args.length - 1) {
                return argType.is(assertion.args[i].type);
            } else {
                return argType.is(lastRequirement.type);
            }
        });
    }

    var j;
    for (j = 0; j < assertions.length; j += 1) {
        var assertion = assertions[j];
        if (matchesAssertion(assertion)) {
            return assertion;
        }
    }
    ...
    that.fail({
        ...
        message: function (output) {
            ...
            var assertionDeclarations = Object.keys(assertions.reduce(function (result, assertion) {
                result[assertion.declaration] = true;
                return result;
            }, {})).sort();

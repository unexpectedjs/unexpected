        ...
        if (definedForIncompatibleTypes.length > 0) {
            ...
            forEach([].concat(this.types).reverse(), function (type) {
                ...
                forEach(getKeys(this.assertions[type.name]), function (assertion) {
                    var distance = levenshteinDistance(testDescriptionString, assertion);
                    distances.push({
                        type: type,
                        assertion: assertion,
                        distance: distance - typeMatchBonus
                    });
                });
            }, this);
            ...
            errorMessage =
                ...
                'did you mean: "' + distances[0].assertion + '"';
        }

        throw truncateStack(new Error(errorMessage));
    }
};

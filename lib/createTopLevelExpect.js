...
expectPrototype._assertTopLevelExpect = function(value = true) {
  if ((this === this._topLevelExpect) !== value) {
    throw new Error(
      `This method only works on the ${
        value
          ? 'top level expect function'
          : 'expect function handed to an assertion'
      }`
    );
  }
};
...
expectPrototype.standardErrorMessage = function(output, options) {
  this._assertTopLevelExpect(false);

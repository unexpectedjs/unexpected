...
function validatePlugin(expect, plugin) {
  if (
...
expectPrototype.child = function() {
  ...
  childExpect.exportPlugin = function(plugin) {
    this.use(plugin);

    if (!validatePlugin(parent, plugin)) {
      return this;
    }

    const exportingExpect = Object.create(childExpect);
    exportingExpect.addAssertion = childExpect.exportAssertion;
    exportingExpect.installTheme = childExpect.exportTheme;
    exportingExpect.addType = childExpect.exportType;
    exportingExpect.addStyle = childExpect.exportStyle;

    if (typeof plugin === 'function') {
      plugin(exportingExpect);
    } else {
      plugin.installInto(exportingExpect);
    }

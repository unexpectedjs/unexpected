...
function findSuffixAssertions(assertionString, assertions) {
    ...
    for (var n = tokens.length - 1; n > 0; n -= 1) {
        ...
        if (
            findSuffixAssertions(tokens.slice(0, n).join(' '), assertions) &&
            suffixAssertions
        ) {
    ...
    var wrappedExpectProto = {
        ...
        _getArgTypes: function(index) {
            ...
            return this.args.map(
                function(arg, index) {
                    var assertionArgType = this.assertionRule.args[
                        Math.min(index, lastIndex)
                    ].type;
                    return utils.findFirst(unexpected.types, function(type) {
                        return type.identify &&
                            type.is(assertionArgType) &&
                            type.identify(arg);
                    });
                },
                this
            ...
            if (!this._assertionIndices) {
                ...
                OUTER: while (true) {
                    if (
                        ...
                        isAssertionArg(
                            currentAssertionRule.args[
                                currentAssertionRule.args.length - 2
                            ]
                        )
                    ) {
                        ...
                        if (suffixAssertions) {
                            for (
                                var i = 0;
                                i < suffixAssertions.length;
                                i += 1
                            ) {
                                if (
                                    suffixAssertions[i].args.some(
                                        isAssertionArg
                                    )
                                ) {
                                    offset += currentAssertionRule.args.length -
                                        1;

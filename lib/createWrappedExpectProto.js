...
module.exports = function createWrappedExpectProto(expect) {
  const wrappedExpectProto = {
    ...
    createOutput: expect.createOutput.bind(expect),
    findTypeOf: expect.findTypeOf.bind(expect),
    findTypeOfWithParentType: expect.findTypeOfWithParentType.bind(expect),
    findCommonType: expect.findCommonType.bind(expect),
    it(...args) {
      if (typeof args[0] === 'string') {
        args[0] = utils.forwardFlags(args[0], this.flags);
      }
      return expect.it(...args);
    },
    diff: expect.diff,
    getType: expect.getType.bind(expect),
    output: expect.output,
    outputFormat: expect.outputFormat.bind(expect),
    format: expect.format,
    withError: expect.withError,

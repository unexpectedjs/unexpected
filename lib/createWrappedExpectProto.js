...
module.exports = function createWrappedExpectProto(unexpected) {
    var wrappedExpectProto = {
        ...
        standardErrorMessage: function (output, options) {
            ...
            options.unexpected = unexpected;
            options.parsed = this.parsed;
            options.originalArgs = this.args;
        ...
        shift: function (subject, assertionIndex) {
            if (arguments.length <= 1) {
                ...
                assertionIndex = -1;
                for (var i = 0 ; i < this.parsed.args.length ; i += 1) {
                    if (this.parsed.args[i].isAssertion) {
                        assertionIndex = i;
                        break;
                    }
                }
                if (typeof assertionIndex !== 'number') {
                    throw new Error('expect.shift only works for assertions declared with an <assertion> type at the end');
                }

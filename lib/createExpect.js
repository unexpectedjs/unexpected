...
expectPrototype.lookupAssertionRule = function(
  ...
  let matches = (value, assertionType, key, relaxed) => {
...
expectPrototype._expect = function expect(context, args) {
  ...
  function executeExpect(context, subject, testDescriptionString, args) {
    ...
    utils.setPrototypeOfOrExtend(wrappedExpect, that);

    wrappedExpect.fail = function(...args) {
    ...
    wrappedExpect.it = (...args) => {
      if (typeof args[0] === 'string') {
        args[0] = utils.forwardFlags(args[0], wrappedExpect.flags);
      }
      return that.it(...args);
    };
...
expectPrototype.callInNestedContext = function(callback) {
  try {

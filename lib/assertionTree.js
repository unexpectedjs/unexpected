...
const leafTextInsert = (node, parts, partIndex) => {
    ...
    const rest = insert(emptyNode, parts, partIndex + 1);
...
const leafInsert = (node, parts, partIndex) => {
    ...
    const rest = insert(emptyNode, parts, partIndex + 1);
    const typeEdge = { value: part, node: rest };
...
const branchTextInsert = (node, parts, partIndex) => {
    const part = parts[partIndex];
    const rest = insert(node.textEdges[part] || emptyNode, parts, partIndex + 1);
    ...
    node.typeEdges.forEach((typeEdge) => {
        if (!foundInsetionPoint) {
            ...
            if (edgeTypeIndex === part.type.index) {
                ...
                const rest = insert(typeEdge.node, parts, partIndex + 1);
                typeEdges.push({ value: typeEdge.value, node: rest});
                foundInsetionPoint = true;
            } else if (edgeTypeIndex < part.type.index) {
                const rest = insert(emptyNode, parts, partIndex + 1);
                typeEdges.push({ value: part, node: rest });
...
const addAssertion = (graph, parts) => insert(graph, parts, 0);

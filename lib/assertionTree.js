...
const inspect = value => util.inspect(value, { depth: 4 });
...
const isVarargs = typeConstraint =>
    typeConstraint.minimum !== 1 || typeConstraint.maximum !== 1;
const isShadowedBy = (currentTypeConstraint, newTypeConstraint) =>
    isVarargs(currentTypeConstraint) &&
    isVarargs(currentTypeConstraint) &&
        ...
        if (!foundInsetionPoint) {
            if (typeEdge.constraint.type === part.type) {
                if (isVarargs(typeEdge.constraint) && isVarargs(part)) {
                    const rest = insert(emptyTree, parts, partIndex + 1, value);
                    typeEdges.push(
                        utils.extend({}, rest, { constraint: part })
                    );
            ...
            if (
                typeEdge.constraint.type !== part.type ||
                !isShadowedBy(typeEdge.constraint, part)
            ) {
                typeEdges.push(typeEdge);
            }
        }
...
const updateFastTrack = (tree, parts) => {
    ...
    const updatedEdge = utils.findFirst(
        tree.typeEdges,
        edge => edge.constraint.type === subjectType
    );
    ...
    currentEdges.forEach(edge => {
        if (
            foundInsetionPoint ||
            subjectType.index < edge.constraint.type.index
        ) {
            edges.push(edge);
    ...
    const fastTrack = utils.extend({}, tree.fastTrack, {
        [assertionString]: edges
    });

    return utils.extend({}, tree, { fastTrack });
};

const addAssertion = (tree, parts, value) =>
    updateFastTrack(insert(tree, parts, 0, value), parts);

const findFrom = (tree, args, offset) => {
    if (!tree) {
        return null;
    }

    if (offset === 0 && tree.fastTrack && typeof args[1].value === "string") {
        const typeEdges = tree.fastTrack[args[1].value];
        const result = typeEdges && findFrom({ typeEdges }, args, 0);
        if (result) {
            return result;
        }
    }
    ...
    if (offset === args.length - 1) {
        return { offset, node: tree };
    }

    const arg = args[offset];
    if (typeof arg.value === "string" && tree.textEdges[arg.value]) {
        return findFrom(tree.textEdges[arg.value], args, offset + 1);
        ...
        for (var i = 0; i < tree.typeEdges.length && !result; i += 1) {
            ...
            if (
                edge.constraint.type.name === "assertion" &&
                typeof arg.value === "string"
            ) {
                return { offset, node: edge };
            }

            if (arg.type.is(edge.constraint.type)) {
                result = findFrom(edge, args, offset + 1);
            }
        }

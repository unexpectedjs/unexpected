...
function isPendingOrHasUnhandledRejection(promise) {
    return promise.isPending() ||
        (promise.isRejected() && promise.reason().uncaught);
}
            ...
            afterEach(function() {
                ...
                if (
                    ...
                    if (this.currentTest) {
                        ...
                        testPassed = currentSpec.failedExpectations.length ===
                            0;
                        displayName = currentSpec.fullName;
                    }
                    error = new Error(
                        displayName +
                            ': You have created a promise that was not returned from the it block'
                    );
                }

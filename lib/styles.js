...
module.exports = expect => {
  ...
  expect.addStyle("merge", function(pens) {
    var flattenedPens = pens.map(pen => flattenBlocksInLines(pen.output)).reverse();
    var maxHeight = flattenedPens.reduce((maxHeight, pen) => Math.max(maxHeight, pen.length), 0);
  ...
  expect.addStyle("arrowsAlongsideChangeOutputs", function(packing, changeOutputs) {
    if (packing) {
      ...
      var arrows = [];
      packing.forEach((columnSet, i, packing) => {
        columnSet.forEach(entry => {
          arrows.push(
            that.clone().arrow({
              left: i * 2,

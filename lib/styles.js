...
module.exports = expect => {
  ...
  expect.addStyle('stringDiffFragment', function(
    ...
    lines.forEach(function(line, i, { length }) {
  ...
  expect.addStyle('stringDiff', function(actual, expected, options = {}) {
    ...
    unifiedDiff(changes, (ch, value, changeInfo = {}) => {
      if (ch === 's') {
        ...
        const inlineChanges = stringDiff[`diff${type}`](oldValue, newValue);
        inlineChanges.forEach(({ added, value, removed }) => {
          if (added) {
        ...
        this.nl()
          .append(newLine)
          .nl(oldEndsWithNewline && !changeInfo.isLastChange ? 1 : 0);
      } else {

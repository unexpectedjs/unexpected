...
module.exports = expect => {
  ...
  expect.addStyle("annotationBlock", function() {
    const pen = this.getContentFromArguments(arguments);
  ...
  expect.addStyle("merge", function(pens) {
    const flattenedPens = pens.map(({output}) => flattenBlocksInLines(output)).reverse();
    const maxHeight = flattenedPens.reduce((maxHeight, {length}) => Math.max(maxHeight, length), 0);

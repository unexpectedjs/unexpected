...
module.exports = function(expect) {
    ...
    expect.addStyle('shouldEqualError', function(expected) {
        this.error(
            typeof expected === 'undefined' ? 'should be' : 'should equal'
        )
            .sp()
            .block(function() {
    ...
    expect.addStyle(
        'stringDiffFragment',
        function(ch, text, baseStyle, markUpSpecialCharacters) {
            text.split(/\n/).forEach(
                function(line, i, lines) {
                    ...
                    if (markUpSpecialCharacters) {
                        line.split(specialCharRegExp).forEach(
                            function(part) {
        ...
        diffLines.forEach(
            function(part, index) {
                if (part.replaced) {
                    ...
                    stringDiff['diff' + type](oldValue, newValue).forEach(
                        function(part) {
                            if (part.added) {
                    ...
                    this.nl()
                        .append(newLine)
                        .nl(
                            oldEndsWithNewline && index < diffLines.length - 1
                                ? 1
                                : 0
                        );
                } else {
                ...
                for (i = 0; i < flattenedPens.length; i += 1) {
                    ...
                    if (currentLine) {
                        ...
                        if (currentBlock) {
                            contentLeft = true;
                            if (!hasOutputChar) {
                                var ch = currentBlock.args.content.charAt(
                                    blockOffsets[i]
                                );
                                if (ch !== ' ') {
                    ...
                    columnSet.forEach(function(entry) {
                        arrows.push(
                            that.clone().arrow({
                                ...
                                height: topByChangeNumber[entry.end] -
                                    topByChangeNumber[entry.start] +
                                    1,

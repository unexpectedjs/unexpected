...
module.exports = function createStandardErrorMessage(output, subject, testDescription, args, options = {}) {
  ...
  if (typeof args === "function") {
    ...
    if (args.length > 0) {
      ...
      args.forEach((arg, index) => {
        const isAssertion = arg && typeof arg === "object" && arg instanceof AssertionString;
        if (0 < index) {
  ...
  const width = preamble.length + subjectSize.width + argsSize.width + testDescription.length;

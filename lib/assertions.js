...
module.exports = function (expect) {
    ...
    expect.addAssertion('<any> [not] to be one of <any>', function (expect, subject, superset) {
        // See what's common between both sets
        var matches = [];
        for (var i = 0; i < superset.length; i += 1) {
            if (subject === superset[i] && matches.indexOf(subject) === -1) {
                matches.push(subject);
            }
        }

        // If all input is in the common set, we're OK
        if ((matches.length === 1) === expect.flags.not) {
            expect.fail({
                message: function (output) {
                    return 'Expected', subject, (expect.flags.not ? 'not ' : ''), 'to be in', superset;
                }
            });
        }
    });

    ...
    expect.addAssertion('function', '[not] to error', function (expect, subject, arg) {
        ...
        return expect.promise(function () {
            ...
            if (that.flags.not) {
                ...
                expect.fail('errored with: {0}', error._isUnexpected ? error.output : expect.inspect(error));
            } else if (hasArg) {
                that.errorMode = 'nested';
                if (error._isUnexpected && (typeof arg === 'string' || isRegExp(arg))) {
                    return expect(error.output.toString(), 'to satisfy', arg);
                } else {

                    ...
                    if (flags.not) {
                        ...
                        ranges.sort(function (a, b) {
                            ...
                            if (range.endIndex > firstUncoveredIndex) {
                                output.match(subject.substring(firstUncoveredIndex, range.endIndex), range.partial);
                ...
                diff: function (output) {
                    if (flags.not) {
                        ...
                        if (i === 0) {
                            ...
                            output.match(subject.substr(0, i)).text(subject.substr(i), true);
                        }
                    }
                    return {diff: output};
                }
            ...
            expect.fail({
                diff: function (output) {
                    if (flags.not) {
                        ...
                        output.text(subject.substr(0, subject.length - i)).match(subject.substr(subject.length - i, subject.length), true);
                    }
                    return {diff: output};
                }
            });

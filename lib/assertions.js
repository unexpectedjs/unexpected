    ...
    expect.addAssertion('Error', 'to have (ansi|html|text|) (message|diff)', function (expect, subject, value) {
        ...
        if (subject.isUnexpected) {
            ...
            if (valueType.is('magicpen')) {
                ...
                expectedOutput = expect.createOutput(format);
                value.call(expectedOutput, subjectPen.toString());
                if (expectedOutput.isEmpty()) {
                    // The function didn't generate any expected output, assume that it ran assertions based on the serialized message
            ...
            if (useDiff) {
                expect.fail('Cannot get the diff representation from a non-Unexpected error');
            }
            if (format !== 'text') {

...
module.exports = function (expect) {
    ...
    expect.addAssertion('<array-like> to [exhaustively] satisfy <array-like>', function (expect, subject, value) {
        var subjectItems = expect.subjectType.unwrap(subject);
        var valueItems = expect.argTypes[0].unwrap(value);
        expect.errorMode = 'bubble';
        var keyPromises = new Array(valueItems.length);
        var i;
        var valueKeys = new Array(valueItems.length);
        for (i = 0 ; i < valueItems.length ; i += 1) {
            valueKeys[i] = i;
            keyPromises[i] = expect.promise(function () {
                var valueKeyType = expect.findTypeOf(valueItems[i]);
                if (valueKeyType.is('function')) {
                    return value[i](subjectItems[i]);
                } else {
                    return expect(subjectItems[i], 'to [exhaustively] satisfy', valueItems[i]);
                }
            });
        }
        return expect.promise.all([
            expect.promise(function () {
                expect(subjectItems, 'to only have keys', valueKeys);
            }),
            expect.promise.all(keyPromises)
        ]).caught(function () {
            var subjectType = expect.subjectType;
            return expect.promise.settle(keyPromises).then(function () {
                var toSatisfyMatrix = new Array(subjectItems.length * valueItems.length);
                for (i = 0 ; i < subjectItems.length ; i += 1) {
                    if (i < valueItems.length) {
                        toSatisfyMatrix[i + i * subjectItems.length] = keyPromises[i].isFulfilled() || keyPromises[i].reason();
                    }
                }
                if (subjectItems.length > 10 || valueItems.length > 10) {
                    ...
                    for (i = 0 ; i < subjectItems.length ; i += 1) {
                        ...
                        if (i < valueItems.length) {
                            indexByIndexChanges.push({
                                ...
                                value: subjectItems[i],
                                expected: valueItems[i],
                                actualIndex: i,
                                expectedIndex: i,
                                last: i === Math.max(subjectItems.length, valueItems.length) - 1
                            });
                        } else {
                            indexByIndexChanges.push({
                                type: 'remove',
                                value: subjectItems[i],
                                actualIndex: i,
                                last: i === subjectItems.length - 1
                            });
                        }
                    }
                    for (i = subjectItems.length ; i < valueItems.length ; i += 1) {
                        indexByIndexChanges.push({
                            type: 'insert',
                            value: value[i],
                            expectedIndex: i
                ...
                var isAsync = false;
                var changes = arrayChanges(subjectItems, valueItems, function equal(a, b, aIndex, bIndex) {
                    var existingResult = toSatisfyMatrix[aIndex + subjectItems.length * bIndex];
                    ...
                    try {
                        ...
                        toSatisfyMatrix[aIndex + subjectItems.length * bIndex] = err;
                        return false;
                    }
                    ...
                    toSatisfyMatrix[aIndex + subjectItems.length * bIndex] = true;
                    return true;
                }, function (a, b) {
                ...
                if (isAsync) {
                    return expect.promise(function (resolve, reject) {
                        arrayChangesAsync(subjectItems, valueItems, function equal(a, b, aIndex, bIndex, cb) {
                            var existingResult = toSatisfyMatrix[aIndex + subjectItems.length * bIndex];
                            if (typeof existingResult !== 'undefined') {
                            ...
                            changes.forEach(function (diffItem, index) {
                                ...
                                output.nl(index > 0 ? 1 : 0).i().block(function () {
                                    ...
                                    if (type === 'insert') {
                                        ...
                                        var toSatisfyResult = toSatisfyMatrix[diffItem.actualIndex + subjectItems.length * diffItem.expectedIndex];

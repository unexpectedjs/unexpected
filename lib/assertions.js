...
module.exports = expect => {
  ...
  expect.addAssertion("<any> [not] to be (ok|truthy) <string>", (expect, subject, message) => {
    var not = !!expect.flags.not;
    var condition = !!subject;
    if (condition === not) {
      expect.fail({
  ...
  expect.addAssertion("<string> [not] to be <string>", (expect, subject, value) => {
  ...
  expect.addAssertion("<any> [not] to be falsy <string>", (expect, subject, message) => {
    var not = !!expect.flags.not;
    var condition = !!subject;
    if (condition !== not) {
      expect.fail({
  ...
  expect.addAssertion("<number> [not] to be close to <number> <number?>", (expect, subject, value, epsilon) => {
  ...
  expect.addAssertion("<any> [not] to be (a|an) <type>", (expect, subject, type) => {
  ...
  expect.addAssertion("<any> [not] to be (a|an) <string>", (expect, subject, typeName) => {
  ...
  expect.addAssertion("<any> [not] to be (a|an) <function>", (expect, subject, Constructor) => {
  ...
  expect.addAssertion("<any> [not] to be one of <array>", (expect, subject, superset) => {
    var found = false;
  ...
  expect.addAssertion("<any> [not] to be an (object|array)", (expect, subject) => {
  ...
  expect.addAssertion("<string> to be (the empty|an empty|a non-empty) string", (expect, subject) => {
    expect(subject, expect.alternations[0] === "a non-empty" ? "not to be empty" : "to be empty");
  });

  expect.addAssertion("<array-like> to be (the empty|an empty|a non-empty) array", (expect, subject) => {
    expect(subject, expect.alternations[0] === "a non-empty" ? "not to be empty" : "to be empty");
  });

  expect.addAssertion("<string> to match <regexp>", (expect, subject, regexp) => expect.withError(
    () => {
  ...
  expect.addAssertion("<string> not to match <regexp>", (expect, subject, regexp) => expect.withError(
    () => {
  ...
  expect.addAssertion("<object|function> [not] to have own property <string>", (expect, subject, key) => {
  ...
  expect.addAssertion("<object|function> [not] to have property <string>", (expect, subject, key) => {
    expect(subject[key], "[!not] to be undefined");
    return subject[key];
  });

  expect.addAssertion("<object|function> to have [own] property <string> <any>", (expect, subject, key, expectedPropertyValue) => expect(subject, "to have [own] property", key).then(actualPropertyValue => {
  ...
  expect.addAssertion("<object|function> [not] to have [own] properties <array>", (expect, subject, propertyNames) => {
  ...
  expect.addAssertion("<object|function> to have [own] properties <object>", (expect, subject, properties) => {
    expect.withError(
      () => {
  ...
  expect.addAssertion("<string|array-like> [not] to have length <number>", (expect, subject, length) => {
    if (!expect.flags.not) {
      expect.errorMode = "nested";
    }
  ...
  expect.addAssertion("<string|array-like> [not] to be empty", (expect, subject) => {
    expect(subject, "[not] to have length", 0);
  });

  expect.addAssertion("<string|array-like|object> to be non-empty", (expect, subject) => {
    expect(subject, "not to be empty");
  });

  expect.addAssertion("<object> to [not] [only] have keys <array>", (expect, subject, keys) => {
    var keysInSubject = {};
    ...
    var hasKeys = keys.every(key => keysInSubject[key]);
  ...
  expect.addAssertion("<object> not to have keys <array>", (expect, subject, keys) => {
    expect(subject, "to not have keys", keys);
  });

  expect.addAssertion("<object> not to have key <string>", (expect, subject, value) => {
  ...
  expect.addAssertion("<object> to [not] [only] have key <string>", (expect, subject, value) => {
    ...
    expect.withError(
      () => {
        args.forEach(arg => {
          expect(
            subject &&
              Array.prototype.some.call(subject, item => expect.equal(item, arg)),
            "[not] to be truthy"
          );
        });
      },
      e => {
        expect.fail({
          diff:
            expect.flags.not &&
            ((output, diff, inspect, equal) => diff(
              subject,
              Array.prototype.filter.call(subject, item => !args.some(arg => equal(item, arg)))
            ))
        });
      }
    );
  ...
  expect.addAssertion("<string> [not] to begin with <string>", (expect, subject, value) => {
    if (value === "") {
      throw new Error(
        "The '" +
  ...
  expect.addAssertion("<string> [not] to end with <string>", (expect, subject, value) => {
    if (value === "") {
      throw new Error(
        "The '" +
          expect.testDescription +
  ...
  expect.addAssertion("<number> [not] to be within <number> <number>", (expect, subject, start, finish) => {
  ...
  expect.addAssertion("<string> [not] to be within <string> <string>", (expect, subject, start, finish) => {
    expect.argsOutput = output => {
      output.appendInspected(start).text("..").appendInspected(finish);
    };
    expect(subject >= start && subject <= finish, "[not] to be truthy");
  ...
  expect.addAssertion("<number> [not] to be (less than|below) <number>", (expect, subject, value) => {
    expect(subject < value, "[not] to be truthy");
  });

  expect.addAssertion("<string> [not] to be (less than|below) <string>", (expect, subject, value) => {
    expect(subject < value, "[not] to be truthy");
  });

  expect.addAssertion("<number> [not] to be less than or equal to <number>", (expect, subject, value) => {
    expect(subject <= value, "[not] to be truthy");
  });

  expect.addAssertion("<string> [not] to be less than or equal to <string>", (expect, subject, value) => {
    expect(subject <= value, "[not] to be truthy");
  });

  expect.addAssertion("<number> [not] to be (greater than|above) <number>", (expect, subject, value) => {
    expect(subject > value, "[not] to be truthy");
  });

  expect.addAssertion("<string> [not] to be (greater than|above) <string>", (expect, subject, value) => {
    expect(subject > value, "[not] to be truthy");
  });

  expect.addAssertion("<number> [not] to be greater than or equal to <number>", (expect, subject, value) => {
    expect(subject >= value, "[not] to be truthy");
  });

  expect.addAssertion("<string> [not] to be greater than or equal to <string>", (expect, subject, value) => {
    expect(subject >= value, "[not] to be truthy");
  });
  ...
  expect.addAssertion("<function> to error", (expect, subject) => expect
    .promise(() => subject())
    .then(
      () => {
        expect.fail();
  ...
  expect.addAssertion("<function> to error [with] <any>", (expect, subject, arg) => expect(subject, "to error").then(error => {
    expect.errorMode = "nested";
  ...
  expect.addAssertion("<function> to (throw|throw error|throw exception)", (expect, subject) => {
    try {
      subject();
    } catch (e) {
      return e;
  ...
  expect.addAssertion("<function> to throw (a|an) <function>", (expect, subject, value) => {
  ...
  expect.addAssertion("<function> to (throw|throw error|throw exception) <any>", (expect, subject, arg) => {
    expect.errorMode = "nested";
    return expect(subject, "to throw").then(error => {
  ...
  expect.addAssertion("<function> to have arity <number>", (expect, subject, value) => {
    ...
    (expect, subject, nextArg) => {
      ...
      keys.forEach((key, index) => {
        if (typeof nextArg === "string") {
          ...
          expected[key] = s => nextArg._expectIt ? nextArg(s, expect.context) : nextArg(s, index);
        } else {
      ...
      return expect.promise
        .any(
          keys.map((key, index) => {
            ...
            return expect.promise(() => expect(subject[key], "to [exhaustively] satisfy", expected));
          })
        )
        .catch(e => expect.fail(output => {
        output.append(expect.standardErrorMessage(output.clone(), { compact: true }));
      }));
    }
  );
  ...
  expect.addAssertion("<Error> to have message <any>", (expect, subject, value) => {
    expect.errorMode = "nested";
    return expect(
      subject.isUnexpected ? subject.getErrorMessage("text").toString() : subject.message,
      "to satisfy",
      value
    );
  });

  expect.addAssertion("<Error> to [exhaustively] satisfy <Error>", (expect, subject, value) => {
  ...
  expect.addAssertion("<Error> to [exhaustively] satisfy <object>", (expect, subject, value) => {
  ...
  expect.addAssertion("<Error> to [exhaustively] satisfy <regexp|string>", (expect, subject, value) => expect(subject.message, "to [exhaustively] satisfy", value));

  expect.addAssertion("<Error> to [exhaustively] satisfy <any>", (expect, subject, value) => expect(subject.message, "to [exhaustively] satisfy", value));

  expect.addAssertion("<binaryArray> to [exhaustively] satisfy <expect.it>", (expect, subject, value) => expect.withError(
  ...
  expect.addAssertion("<UnexpectedError> to [exhaustively] satisfy <function>", (expect, subject, value) => expect.promise(() => {
    subject.serializeMessage(expect.outputFormat());
    return value(subject);
  }));

  expect.addAssertion("<any|Error> to [exhaustively] satisfy <function>", (expect, subject, value) => expect.promise(() => value(subject)));

  if (typeof Buffer !== "undefined") {
    expect.addAssertion("<Buffer> [when] decoded as <string> <assertion?>", (expect, subject, value) => expect.shift(subject.toString(value)));
  }

  expect.addAssertion("<any> not to [exhaustively] satisfy [assertion] <any>", (expect, subject, value) => expect.promise((resolve, reject) => expect
    .promise(() => expect(subject, "to [exhaustively] satisfy [assertion]", value))
    .then(() => {
      try {
        expect.fail();
      } catch (e) {
  ...
  expect.addAssertion("<any> to [exhaustively] satisfy assertion <any>", (expect, subject, value) => {
  ...
  expect.addAssertion("<any> to [exhaustively] satisfy assertion <assertion>", (expect, subject) => {
    expect.errorMode = "bubble"; // to satisfy assertion 'to be a number' => to be a number
    return expect.shift();
  });

  expect.addAssertion("<any> to [exhaustively] satisfy [assertion] <expect.it>", (expect, subject, value) => expect.withError(
    () => value(subject, expect.context),
    e => {
      expect.fail({
        diff: function(output) {
  ...
  expect.addAssertion("<regexp> to [exhaustively] satisfy <regexp>", (expect, subject, value) => {
    expect(subject, "to equal", value);
  });

  expect.addAssertion("<string> to [exhaustively] satisfy <regexp>", (expect, subject, value) => {
    expect.errorMode = "bubble";
    return expect(subject, "to match", value);
  });

  expect.addAssertion("<function> to [exhaustively] satisfy <function>", (expect, subject, value) => {
    expect.errorMode = "bubble";
    expect(subject, "to equal", value);
  });

  expect.addAssertion("<binaryArray> to [exhaustively] satisfy <binaryArray>", (expect, subject, value) => {
    expect.errorMode = "bubble";
    expect(subject, "to equal", value);
  });

  expect.addAssertion("<any> to [exhaustively] satisfy <any>", (expect, subject, value) => {
    expect.errorMode = "bubble";
    expect(subject, "to equal", value);
  });

  expect.addAssertion("<array-like> to [exhaustively] satisfy <array-like>", (expect, subject, value) => {
    expect.errorMode = "bubble";
    var i;
    var valueType = expect.argTypes[0];
    var valueKeys = valueType.getKeys(value);
    ...
    valueKeys.forEach(valueKey => {
      keyPromises[valueKey] = expect.promise(() => {
      ...
      .caught(() => {
        ...
        return expect.promise.settle(keyPromises).then(() => {
          ...
          var nonNumericalKeysAndSymbols =
            ...
            utils.uniqueNonNumericalStringsAndSymbols(
              subjectType.getKeys(subject),
              valueType.getKeys(value)
            );
              ...
              .promise((resolve, reject) => {
                arrayChangesAsync(
                  ...
                  function equal(a, b, aIndex, bIndex, cb) {
                    ...
                    expect
                      .promise(() => expect(a, "to [exhaustively] satisfy", b))
                      .then(
                        () => {
                          toSatisfyMatrix[aIndex][bIndex] = true;
                          cb(true);
          ...
          function failWithChanges(changes) {
            ...
            expect.fail({
              diff: function(output, diff, inspect, equal) {
                output.inline = true;
                var indexOfLastNonInsert = changes.reduce((previousValue, diffItem, index) => diffItem.type === "insert" ? previousValue : index, -1);
                        ...
                        if (type === "moveSource") {
                          ...
                          this.annotationBlock(function() {
                            ...
                            if (expect.findTypeOf(diffItem.value).is("function")) {
                              this.error("missing: ").property(
                                index,
                                output.clone().block(function() {
                          ...
                          this.property(
                            ...
                            output.clone().block(function() {
                              if (type === "remove") {
                                ...
                                var toSatisfyResult =
                                  toSatisfyMatrix[diffItem.actualIndex][diffItem.expectedIndex];
                                var valueDiff =
                                  toSatisfyResult &&
                                  toSatisfyResult !== true &&
                                  toSatisfyResult.getDiff({ output: output.clone() });
  ...
  expect.addAssertion("<object> to [exhaustively] satisfy <object>", (expect, subject, value) => {
    var valueType = expect.argTypes[0];
    var subjectType = expect.subjectType;
    var subjectIsArrayLike = subjectType.is("array-like");
    if (subject === value) {
    ...
    return expect.promise
      .all([
        expect.promise(() => {
          if (expect.flags.exhaustively) {
            var nonOwnKeysWithDefinedValues = keys.filter(key => !Object.prototype.hasOwnProperty.call(subject, key) &&
            typeof subject[key] !== "undefined");
            var valueKeysWithDefinedValues = keys.filter(key => typeof value[key] !== "undefined");
            var subjectKeysWithDefinedValues = subjectKeys.filter(key => typeof subject[key] !== "undefined");
            expect(
              valueKeysWithDefinedValues.length - nonOwnKeysWithDefinedValues.length,
              "to equal",
      ...
      .caught(() => expect.promise.settle(promiseByKey).then(() => {
      expect.fail({
        diff: function(output, diff, inspect, equal) {
          output.inline = true;
          var subjectIsArrayLike = subjectType.is("array-like");
          var keys = utils
            .uniqueStringsAndSymbols(subjectKeys, valueType.getKeys(value))
            .filter(key => // Skip missing keys expected to be missing so they don't get rendered in the diff
          key in subject || typeof value[key] !== "undefined");
          var prefixOutput = subjectType.prefix(output.clone(), subject);
          output.append(prefixOutput).nl(prefixOutput.isEmpty() ? 0 : 1);
          ...
          keys.forEach((key, index) => {
            output.nl(index > 0 ? 1 : 0).i().block(function() {
              ...
              if (
                Object.prototype.hasOwnProperty.call(promiseByKey, key) &&
              ...
              if (!(key in value)) {
                ...
                if (keyDiff && keyDiff.inline) {
                  ...
                  annotation
                    .error((conflicting && conflicting.getLabel()) || "should satisfy")
                    .sp()
                    .block(inspect(value[key]));
              ...
              var annotationOnNextLine =
                ...
                output.preferredWidth <
                  this.size().width + valueOutput.size().width + annotation.size().width;
  ...
  expect.addAssertion("<wrapperObject> to [exhaustively] satisfy <wrapperObject>", (expect, subject, value) => {
  ...
  expect.addAssertion("<wrapperObject> to [exhaustively] satisfy <any>", (expect, subject, value) => {
    var subjectType = expect.subjectType;
  ...
  expect.addAssertion("<function> [when] called with <array-like> <assertion?>", (expect, subject, args) => {
    // ...
    expect.errorMode = "nested";
    expect.argsOutput[0] = output => {
  ...
  expect.addAssertion("<function> [when] called <assertion?>", (expect, subject) => {
    expect.errorMode = "nested";
  ...
  expect.addAssertion(
    ...
    (expect, subject, value) => {
      expect.errorMode = "nested";
      return expect(subject, "to be rejected").tap(err => expect.withError(
        () => {
  ...
  expect.addAssertion(
    ...
    (expect, subject, value) => {
      expect.errorMode = "nested";
      return expect(subject, "to be fulfilled").tap(fulfillmentValue => expect(fulfillmentValue, "to [exhaustively] satisfy", value));
    }
  );
  ...
  expect.addAssertion("<Promise> when rejected <assertion>", (expect, subject, nextAssertion) => {
    expect.errorMode = "nested";
    return expect
      .promise(() => subject)
      .then(
  ...
  expect.addAssertion("<function> when rejected <assertion>", (expect, subject, ...rest) => {
    expect.errorMode = "nested";
    return expect(
      expect.promise(() => subject()),
      "when rejected",
  ...
  expect.addAssertion("<Promise> when fulfilled <assertion>", (expect, subject, nextAssertion) => {
    expect.errorMode = "nested";
    return expect
      .promise(() => subject)
      .then(
  ...
  expect.addAssertion("<function> when fulfilled <assertion>", (expect, subject, ...rest) => {
    expect.errorMode = "nested";
    return expect(
      expect.promise(() => subject()),
      "when fulfilled",
  ...
  expect.addAssertion("<function> to call the callback without error", (expect, subject) => expect(subject, "to call the callback").then(callbackArgs => {
  ...
  expect.addAssertion("<function> to call the callback with error", (expect, subject) => expect(subject, "to call the callback").spread(err => {
    expect(err, "to be truthy");
    return err;
  }));

  expect.addAssertion("<function> to call the callback with error <any>", (expect, subject, value) => expect(subject, "to call the callback with error").tap(err => {
    expect.errorMode = "nested";
    if (err && err._isUnexpected && (typeof value === "string" || isRegExp(value))) {
      return expect(err, "to have message", value);
    } else {

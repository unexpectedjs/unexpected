...
module.exports = function (expect) {
    ...
    expect.addAssertion('<array-like> to [exhaustively] satisfy <array-like>', function (expect, subject, value) {
        ...
        var valueType = expect.argTypes[0];
        var unwrappedSubject = expect.subjectType.unwrap(subject);
        var unwrappedValue = valueType.unwrap(value);
        var valueKeys = valueType.getKeys(value);
        var keyPromises = {};
        valueKeys.forEach(function (valueKey) {
            keyPromises[valueKey] = expect.promise(function () {
                var valueKeyType = expect.findTypeOf(unwrappedValue[valueKey]);
                if (valueKeyType.is('function')) {
                    return value[valueKey](unwrappedSubject[valueKey]);
                } else {
                    return expect(unwrappedSubject[valueKey], 'to [exhaustively] satisfy', unwrappedValue[valueKey]);
                }
            });
        });
        return expect.promise.all([
            ...
            return expect.promise.settle(keyPromises).then(function () {
                var toSatisfyMatrix = new Array(unwrappedSubject.length);
                for (i = 0 ; i < unwrappedSubject.length ; i += 1) {
                    toSatisfyMatrix[i] = new Array(unwrappedValue.length);
                    if (i < unwrappedValue.length) {
                        toSatisfyMatrix[i][i] = keyPromises[i].isFulfilled() || keyPromises[i].reason();
                    }
                }
                if (unwrappedSubject.length > 10 || unwrappedValue.length > 10) {
                    ...
                    for (i = 0 ; i < unwrappedSubject.length ; i += 1) {
                        ...
                        if (i < value.length) {
                            indexByIndexChanges.push({
                                ...
                                value: unwrappedSubject[i],
                                expected: unwrappedValue[i],
                                actualIndex: i,
                                expectedIndex: i,
                                last: i === Math.max(unwrappedSubject.length, unwrappedValue.length) - 1
                            });
                        } else {
                            indexByIndexChanges.push({
                                type: 'remove',
                                value: unwrappedSubject[i],
                                actualIndex: i,
                                last: i === unwrappedSubject.length - 1
                            });
                        }
                    }
                    for (i = unwrappedSubject.length ; i < unwrappedValue.length ; i += 1) {
                        indexByIndexChanges.push({
                            type: 'insert',
                            value: value[i],
                            expectedIndex: i
                ...
                var changes = arrayChanges(unwrappedSubject, unwrappedValue, function equal(a, b, aIndex, bIndex) {
                ...
                if (isAsync) {
                    return expect.promise(function (resolve, reject) {
                        arrayChangesAsync(unwrappedSubject, unwrappedValue, function equal(a, b, aIndex, bIndex, cb) {
                            toSatisfyMatrix[aIndex] = toSatisfyMatrix[aIndex] || [];
                            var existingResult = toSatisfyMatrix[aIndex][bIndex];
                            if (typeof existingResult !== 'undefined') {
    ...
    expect.addAssertion('<object> to [exhaustively] satisfy <object>', function (expect, subject, value) {
        ...
        var unwrappedSubject = subjectType.unwrap(subject);
        var unwrappedValue = valueType.unwrap(value);

        if (!subjectIsArrayLike) {
            // Find all non-enumerable subject keys present in value, but not returned by subjectType.getKeys:
            keys.forEach(function (key) {
                if (Object.prototype.hasOwnProperty.call(unwrappedSubject, key) && subjectKeys.indexOf(key) === -1) {
        ...
        keys.forEach(function (key, index) {
            promiseByKey[key] = expect.promise(function () {
                var valueKeyType = expect.findTypeOf(unwrappedValue[key]);
                if (valueKeyType.is('function')) {
                    return unwrappedValue[key](unwrappedSubject[key]);
                } else {
                    return expect(unwrappedSubject[key], 'to [exhaustively] satisfy', unwrappedValue[key]);
                }
            });
        });
            ...
            expect.promise(function () {
                if (expect.flags.exhaustively) {
                    var nonOwnKeysWithDefinedValues = keys.filter(function (key) {
                        return !Object.prototype.hasOwnProperty.call(unwrappedSubject, key) && typeof subject[key] !== 'undefined';
                    });
                    var valueKeysWithDefinedValues = keys.filter(function (key) {
                        return typeof unwrappedValue[key] !== 'undefined';
                    });
                    var subjectKeysWithDefinedValues = subjectKeys.filter(function (key) {
                        return typeof unwrappedSubject[key] !== 'undefined';
                    });
                ...
                expect.fail({
                    diff: function (output, diff, inspect, equal) {
                        ...
                        var keys = utils.uniqueStringsAndSymbols(subjectKeys, valueType.getKeys(value)).filter(function (key) {
                            ...
                            return key in unwrappedSubject || typeof unwrappedValue[key] !== 'undefined';
                        });
                        var prefixOutput = subjectType.prefix(output.clone(), subject);
                        output
                            .append(prefixOutput)
                        ...
                        keys.forEach(function (key, index) {
                            output.nl(index > 0 ? 1 : 0).i().block(function () {
                                ...
                                var missingArrayIndex = subjectType.is('array-like') && !(key in unwrappedSubject);

                                var isInlineDiff = true;

                                output.omitSubject = unwrappedSubject[key];
                                if (!(key in unwrappedValue)) {
                                    if (expect.flags.exhaustively) {
                                        annotation.error('should be removed');
                                    } else {
                                        conflicting = null;
                                    }
                                } else if (!(key in unwrappedSubject)) {
                                    if (expect.findTypeOf(unwrappedValue[key]).is('function')) {
                                        ...
                                        output.error('// missing').sp();
                                        valueOutput = inspect(unwrappedValue[key]);
                                    }
                                    ...
                                    if (keyDiff && keyDiff.inline) {
                                        valueOutput = keyDiff;
                                    } else if (typeof unwrappedValue[key] === 'function') {
                                        isInlineDiff = false;
                                        ...
                                        annotation.error((conflicting && conflicting.getLabel()) || 'should satisfy').sp()
                                            .block(inspect(unwrappedValue[key]));
                                ...
                                if (!valueOutput) {
                                    if (missingArrayIndex || !(key in unwrappedSubject)) {
                                        valueOutput = output.clone();
                                    } else {
                                        valueOutput = inspect(unwrappedSubject[key]);
                                    }
                                }

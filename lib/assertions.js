...
module.exports = function(expect) {
    ...
    expect.addAssertion(
        '<any> [not] to be a (boolean|number|string|function|regexp|regex|regular expression)',
        function(expect, subject) {
    ...
    expect.addAssertion(
        ...
        function(expect, subject, regexp) {
                    ...
                    expect.fail({
                        ...
                        diff: function(output) {
                            ...
                            function flushUntilIndex(i) {
                                if (i > lastIndex) {
                                    output.text(
                                        subject.substring(lastIndex, i)
                                    );
                                    lastIndex = i;
                                }
                            }
                            subject.replace(
                                new RegExp(regexp.source, 'g'),
                                function($0, index) {
    ...
    expect.addAssertion(
        ...
        function(expect, subject, key, expectedPropertyValue) {
            return expect(
                ...
                key
            ).then(function(actualPropertyValue) {
                    ...
                    expect.fail({
                        diff: function(output, diff) {
                            ...
                            propertyNames.forEach(function(propertyName) {
                                if (
                                    (!expect.flags.own ||
                                        subject.hasOwnProperty(propertyName)) &&
                                    !(propertyName in properties)
                                ) {
                                    expected[propertyName] = subject[
                                        propertyName
                                    ];
                                }
                                if (
        ...
        function(expect, subject, keys) {
            ...
            if (expect.flags.only) {
                ...
                expect.withError(
                    function() {
                        expect(
                            subjectKeys.length === keys.length,
                            '[not] to be truthy'
                        );
                    },
                    function(err) {
                        expect.fail({
                            diff: !expect.flags.not &&
                                function(output, diff, inspect, equal) {
                                    ...
                                    var subjectType = expect.findTypeOf(
                                        subject
                                    );
                                    var subjectIsArrayLike = subjectType.is(
                                        'array-like'
                                    );
                            ...
                            if (expect.flags.not) {
                                subject.replace(
                                    new RegExp(
                                        args
                                            .map(function(arg) {
                                                return utils.escapeRegExpMetaChars(
                                                    arg
                                                );
                                            })
                                            .join('|'),
                                ...
                                ranges
                                    ...
                                    .forEach(function(range) {
                                        ...
                                        if (
                                            range.endIndex > firstUncoveredIndex
                                        ) {
                                            if (range.partial) {
                                                output.partialMatch(
                                                    subject.substring(
    ...
    expect.addAssertion(
        ...
        function(expect, subject) {
            ...
            return expect.apply(
                expect,
                [keys, 'to have items satisfying'].concat(extraArgs)
            );
        }
    );
            ...
            return expect.withError(
                ...
                function(e) {
                    expect.fail({
                        diff: function(output, diff) {
                            output.inline = false;
                            var unwrappedSubject = expect.subjectType.unwrap(
                                subject
                            );
                            return utils.wrapConstructorNameAroundOutput(
                ...
                .caught(function() {
                    ...
                    return expect.promise.settle(keyPromises).then(function() {
                        ...
                        for (i = 0; i < subject.length; i += 1) {
                            ...
                            if (i < value.length) {
                                toSatisfyMatrix[i][i] = keyPromises[
                                    i
                                ].isFulfilled() || keyPromises[i].reason();
                            }
                        }
                        ...
                        var changes = arrayChanges(
                            ...
                            function equal(a, b, aIndex, bIndex) {
                                toSatisfyMatrix[aIndex] = toSatisfyMatrix[
                                    aIndex
                                ] || [];
                                var existingResult = toSatisfyMatrix[aIndex][
                                    bIndex
                                ];
                                ...
                                .promise(function(resolve, reject) {
                                    arrayChangesAsync(
                                        ...
                                        function equal(
                                            ...
                                            toSatisfyMatrix[
                                                aIndex
                                            ] = toSatisfyMatrix[aIndex] || [];
                                            var existingResult = toSatisfyMatrix[
                                                aIndex
                                            ][bIndex];
                                            if (
                                                    ...
                                                    .block(function() {
                                                        if (
                                                            ...
                                                            this.annotationBlock(
                                                                function() {
                                                                    var index = typeof diffItem.actualIndex !==
                                                                        'undefined'
                                                                        ? diffItem.actualIndex
                                                                    ...
                                                                    .block(
                                                                        function() {
                                                                            if (
                                                                                ...
                                                                                var toSatisfyResult = toSatisfyMatrix[
                                                                                    diffItem.actualIndex
                                                                                ][
                                                                                    diffItem.expectedIndex
                                                                                ];
                                                                                var valueDiff = toSatisfyResult &&
                                                                                    toSatisfyResult !==
                                                                                        true &&
                ...
                .all([
                    expect.promise(function() {
                        if (expect.flags.exhaustively) {
                            ...
                            var valueKeysWithDefinedValues = keys.filter(
                                function(key) {
                                    ...
                                    output
                                        ...
                                        .block(function() {
                                            ...
                                            if (
                                                ...
                                                conflicting = promiseByKey[
                                                    key
                                                ].reason();
                                            }

                                            var missingArrayIndex = subjectType.is(
                                                'array-like'
                                            ) && !(key in subject);
                                            ...
                                            if (!(key in value)) {
                                                ...
                                                conflicting || missingArrayIndex
                                            ) {
                                                var keyDiff = conflicting &&
                                                    conflicting.getDiff({
                                                        output: output
                                                    });
                                                isInlineDiff = !keyDiff ||
                                                    keyDiff.inline;
                                                ...
                                                if (keyDiff && keyDiff.inline) {
                                                    ...
                                                    isInlineDiff = false;
                                                    annotation.appendErrorMessage(
                                                        conflicting
                                                    );
                                                } else if (
                                                    !keyDiff ||
                                                    (keyDiff && !keyDiff.inline)
                                                ) {
                                                    annotation
                                                        .error(
                                                            (conflicting &&
                                                                conflicting.getLabel()) ||
                                                                'should satisfy'
                                                        )
                                                        .sp()
                                                        .block(
                                            ...
                                            var omitDelimiter = missingArrayIndex ||
    ...
    expect.addAssertion(
        ...
        function(expect, subject, compareFunction) {
            ...
            return expect.shift(
                Array.prototype.slice.call(subject).sort(compareFunction)
            );
        }
    );

            ...
            if (hasArg) {
                if (typeof arg === 'function') {
                    try {
                        ...
                        expect.fail(argError);
                    }
                } else if (isUnexpected && (typeof arg === 'string' || isRegExp(arg))) {
    ...
    expect.addAssertion('[not] to [exhaustively] satisfy [assertion]', function (expect, subject, value) {
        if (this.flags.not) {
            ...
            try {
                ...
                // TODO Should we have the diff for the top-level
                if (e._isUnexpected) {
                    e.createDiff = function (output, diff, inspect, equal) {
                        return {
                            diff: output.append(e.output),
                        ...
                        e.createDiff = function (output, diff, inspect, equal) {
                            ...
                            keys.forEach(function (key, index) {
                                output.i().block(function () {
                                    ...
                                    var conflicting;
                                    var isExpectIt = value[key] && value[key]._expectIt;
                                    try {
                                        if (isExpectIt) {

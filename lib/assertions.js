...
module.exports = function (expect) {
    ...
    function instantiate(Constructor, args) {
        if (args.length === 0) {
            ...
            for (var i = 0 ; i < args.length ; i += 1) {
                parameterNames.push('args[' + i + ']');
            }
            /*jshint -W054*/
            return new Function('Constructor', 'args', 'return new Constructor(' + parameterNames.join(', ') + ');')(Constructor, args);
            /*jshint +W054*/
        }
    }

    expect.addAssertion('array', 'when passed as parameters to [async] [constructor]', function (expect, subject, fn) { // ...
        this.errorMode = 'nested';
        var that = this,
            args = subject;
        return expect.promise(function (run) {
            if (that.flags.async) {
                ...
                args.push(run(function (err, result) {
                    ...
                    return that.shift(expect, result, 1);
                }));
                if (that.flags.constructor) {
                    instantiate(fn, args);
                } else {
                    fn.apply(null, args);
                }
    ...
    expect.addAssertion('when passed as parameter to [async] [constructor]', function (expect, subject) {

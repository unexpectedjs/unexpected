...
module.exports = function(expect) {
    ...
    expect.addAssertion(
        ...
        function(expect, subject) {
            expect(
                subject,
                expect.alternations[0] === 'a non-empty' ? 'not to be empty' : 'to be empty'
            );
        }
    );
        ...
        function(expect, subject) {
            ...
            return expect.withError(
                function() {
                    return expect.apply(
                        expect,
                        [subject, 'to have values [exhaustively] satisfying'].concat(extraArgs)
                    );
                },
                function(err) {
                    expect.fail({
        ...
        function(expect, subject) {
            ...
            return expect.withError(
                function() {
                    return expect.apply(
                        expect,
                        [subject, 'to have a value [exhaustively] satisfying'].concat(extraArgs)
                    );
                },
                function(err) {
                    expect.fail(function(output) {
                ...
                .caught(function() {
                    ...
                    return expect.promise.settle(keyPromises).then(function() {
                        ...
                        for (i = 0; i < subject.length; i += 1) {
                            ...
                            if (i < value.length) {
                                toSatisfyMatrix[i][i] = keyPromises[i].isFulfilled() ||
                                    keyPromises[i].reason();
                            }
                        }
                            ...
                            expect.fail({
                                diff: function(output, diff, inspect, equal) {
                                    ...
                                    var indexOfLastNonInsert = changes.reduce(
                                        function(previousValue, diffItem, index) {
                            ...
                            diff: function(output, diff, inspect, equal) {
                                ...
                                keys.forEach(function(key, index) {
                                    output.nl(index > 0 ? 1 : 0).i().block(function() {
                                        ...
                                        var missingArrayIndex = subjectType.is('array-like') &&
                                            !(key in subject);
    ...
    expect.addAssertion(
        ...
        function(expect, subject, fn) {
            ...
            if (expect.flags.async) {
                ...
                return expect.shift(
                    expect.flags.constructor ? instantiate(fn, args) : fn.apply(fn, args)
                );
            }
        }
    );

...
module.exports = function (expect) {
    ...
    expect.addAssertion('<any> [not] to be', function (expect, subject, value) {
    ...
    // NOTE (v10): Will <type> turn into the name or the type instance?
    expect('<any> [not] to be (a|an) <type>', function (expect, subject, type) {
    ...
    expect('<any> [not] to be (a|an) <string>', function (expect, subject, typeName) {
        ...
        expect(subjectType.is(typeName), '[not] to be truthy');
    });

    expect('<any> [not] to be (a|an) <function>', function (expect, subject, Constructor) {
    ...
    expect.addAssertion('<object> [not] to have own property <string>', function (expect, subject, key) {
        expect(subject.hasOwnProperty(key), '[not] to be truthy');
        return subject[key];
    });

    expect.addAssertion('<object> [not] to have property <string>', function (expect, subject, key) {
        expect(subject && subject[key] !== undefined, '[not] to be truthy');
        return subject[key];
    });

    expect.addAssertion('<object> to have [own] property <string> <any>', function (expect, subject, key, expectedPropertyValue) {
    ...
    expect.addAssertion('<object> [not] to have [own] properties <array>', function (expect, subject, properties) {
        properties.forEach(function (property) {
            expect(subject, '[not] to have [own] property', property);
        });
    });

    expect.addAssertion('<object> to have [own] properties <object>', function (expect, subject, properties) {
        expect.withError(function () {
    ...
    expect.addAssertion('<object> [not] [only] to have key <string>', function (expect, subject, value) {
        expect(subject, 'to [not] [only] have keys', [ value ]);
    });

    expect.addAssertion('<object> [not] [only] to have keys <string+>', function (expect, subject) {
        expect(subject, 'to [not] [only] have keys', Array.prototype.slice.call(arguments, 2));
    });

    // NOTE (v10): To be compatible with v9 this would have to support <...any> as the RHS
    ...
    // NOTE (v10): To be compatible with v9 this would have to support <any> as the RHS
    // NOTE (v10): Presently inconsistent with the "[not] to contain" assertions
    ...
    expect.addAssertion('<number|string> [not] to be within <number> <number>', function (expect, subject, start, finish) {
    ...
    expect.addAssertion('<string> [not] to be (greater than|above) <number>', function (expect, subject, value) {
        expect(subject > value, '[not] to be truthy');
    });
    ...
    // NOTE (v10): Split up further?
    expect.addAssertion('<function> to error <any>', function (expect, subject, arg) {
        return expect(subject, 'to error').then(function (error) {
            expect.errorMode = 'nested';
            // NOTE (v10): Hmm, is this already covered by the "<Error> to satisfy..." variants?
    ...
    expect.addAssertion('<function> [not] to (throw|throw error|throw exception) <any>', function (expect, subject, arg) {
    ...
    // NOTE (v10): Unsure about how the assertion will look:
    expect.addAssertion([
    ...
    expect.addAssertion('<Error> to [exhaustively] satisfy <Error>', function (expect, subject, value) {
        ...
        // NOTE (v10): If the types themselves are made available to the handler, the expect.findTypeOf below can be avoided:
        expect(subject, 'to have properties', expect.findTypeOf(value).unwrap(value));
    });

    // NOTE (v10): Check if this is covered by <any> to satisfy <function> below...
    expect.addAssertion('<Error> to [exhaustively] satisfy <function>', function (expect, subject, value) {
        return expect.promise(function () {
    ...
    // NOTE (v10): Check if this is covered by <any> to satisfy <expect.it> below...
    ...
    // NOTE (v10): Check if this is covered by <any> to satisfy <function> below...
    expect.addAssertion('<binaryArray> to [exhaustively] satisfy <function>', function (expect, subject, value) {
        return expect.promise(function () {
            return value(subject);
        });
    });

    expect.addAssertion('<binaryArray> to [exhaustively] satisfy <any>', function (expect, subject, value) {
        expect(subject, 'to equal', value);
    });

    if (typeof Buffer !== 'undefined') {
        expect.addAssertion('<Buffer> when decoded as <string>', function (expect, subject, value) {
    ...
    expect.addAssertion('<any> to [exhaustively] satisfy assertion <string>', function (expect, subject) {
    ...
    // NOTE (v10): Why is there a difference between these two (extracted from the original code), can we get down to one?
    expect.addAssertion('<any> to [exhaustively] satisfy assertion <expect.it>', function (expect, subject) {
        expect.errorMode = 'bubble'; // to satisfy assertion 'to be a number' => to be a number
    ...
    expect.addAssertion('<any> to [exhaustively] satisfy <expect.it>', function (expect, subject, value) {
        return expect.withError(function () {
            return value(subject);
        }, function (e) {
            expect.fail({
    ...
    expect.addAssertion('<any> to [exhaustively] satisfy <function>', function (expect, subject, value) {
        return expect.promise(function () {
            return value(subject);
        });
    });

    // NOTE: An interesting side effect of requiring a <string> subject here is that
    // expect(regexp, 'to satisfy', regexp) will hit the <any> to [exhaustively] satisfy <any> case below
    // Discuss whether we want that.
    ...
    expect.addAssertion('<object> to [exhaustively] satisfy <array-like|object>', function (expect, subject, value) {
        var valueType = expect.findTypeOf(value); // NOTE (v10): Exposed somewhere on expect?
        var subjectType = expect.findTypeOf(subject);
        ...
        keys.forEach(function (key, index) {
            promiseByKey[key] = expect.promise(function () {
                ...
                // NOTE (v10): Is it necessary to special case functions (and implicitly expect.it) here?
    ...
    expect.addAssertion('<function> when called with <array-like>', function (expect, subject, args) { // ...
        expect.errorMode = 'nested';
    ...
    expect.addAssertion([
        '<array-like> when passed as parameters to [async] <function>',
        '<array-like> when passed as parameters to [constructor] <function>'
    ...
    expect.addAssertion([
        '<any> when passed as parameter to [async] <function>',
        '<any> when passed as parameter to [constructor] <function>'
    ], function (expect, subject, fn) { // ...
        expect.errorMode = 'nested';
        var args = [subject];
        if (expect.flags.async) {
    ...
    expect.addAssertion('<Promise> when rejected <any>', function (expect, subject, nextAssertion) {
        expect.errorMode = 'nested';
        return subject.then(function (fulfillmentValue) {
    ...
    expect.addAssertion('<Promise> when fulfilled <any>', function (expect, subject, nextAssertion) {
        expect.errorMode = 'nested';
        return subject.then(function (fulfillmentValue) {
    ...
    expect.addAssertion('<function> to call the callback without error', function (expect, subject) {
        return expect(subject, 'to call the callback').then(function (callbackArgs) {
            expect(callbackArgs[0], 'to be falsy');

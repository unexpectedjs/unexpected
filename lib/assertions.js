...
module.exports = function (expect) {
    ...
    expect.addAssertion([
        ...
        var expected = {};
        var setExpected = function (key, value) {
            expected[key] = value;
        };
        // in the case of a Map we ensure we can store complex keys
        if (typeof Map === 'function' && subjectType.is('Map')) {
            expected = new Map();
            setExpected = function (key, value) {
    ...
    expect.addAssertion('<object> to [exhaustively] satisfy <object>', function (expect, subject, value) {
        ...
        var subjectKeys = subjectType.getKeys(subject);
        var uniqueKeys = subjectType.uniqueKeys(subjectKeys, keys);
        ...
        var hasSubjectOverride = (typeof subjectType.valueFromKey === 'function');
        var hasValueOverride = (typeof valueType.valueFromKey === 'function');

        var promisesForOtherKeys = [];
        var promisesForUniqueKeys = {};

        keys.forEach(function (key) {
            var promise = expect.promise(function () {
                var subjectKey = hasSubjectOverride ? subjectType.valueFromKey(subject, key) : subject[key];
                var valueKey = hasValueOverride ? valueType.valueFromKey(value, key) : value[key];
            ...
            var uniqueKeyIndex = uniqueKeys.indexOf(key);
            if (uniqueKeyIndex !== -1) {
                promisesForUniqueKeys[uniqueKeyIndex] = promise;
            } else if (subjectIsArrayLike) {
                // treat the key is an array index
                uniqueKeyIndex = uniqueKeys.indexOf(Number(key));
                promisesForUniqueKeys[uniqueKeyIndex] = promise;
            } else {
                promisesForOtherKeys.push(promise);
            }
        });

        var promisesForKeys = [].concat(Object.keys(promisesForUniqueKeys).map(function (key) {
            return promisesForUniqueKeys[key];
        }), promisesForOtherKeys);
        ...
        return expect.promise.all([
            ...
            expect.promise.all(promisesForKeys)
        ]).caught(function () {
            return expect.promise.settle(promisesForKeys).then(function () {
                expect.fail({
                    diff: function (output, diff, inspect, equal) {
                        output.inline = true;
                        var subjectIsArrayLike = subjectType.is('array-like');
                        ...
                        uniqueKeys.forEach(function (key, index) {
                            var displayIndex = index - skippedCount;
                            var valueKey = hasValueOverride ? valueType.valueFromKey(value, key) : value[key];

                            // Skip missing keys expected to be missing so they don't get rendered in the diff
                            if (!(subjectType.hasKey(subject, key) || typeof valueKey !== 'undefined')) {
                                skippedCount += 1;
                                return;
                            }
                            ...
                            output.nl(displayIndex > 0 ? 1 : 0).i().block(function () {
                                var valueOutput;
                                var annotation = output.clone();
                                var conflicting;

                                var promise = promisesForUniqueKeys[index];
                                if (promise !== undefined && promise.isRejected()) {
                                    conflicting = promise.reason();
                                }
                                ...
                                if (!valueType.hasKey(value, key)) {
                                    ...
                                    if (expect.findTypeOf(valueKey).is('function')) {
                                        if (promise.isRejected()) {
                                            ...
                                            valueOutput = output.clone().appendErrorMessage(promise.reason());
                                        } else {
                                            output.error('// missing').sp();
                                ...
                                var omitDelimiter =
                                    missingArrayIndex ||
                                    displayIndex >= subjectKeys.length - 1;

                                if (!omitDelimiter || subjectType.delimitAlways) {
                                    valueOutput.amend(subjectType.delimiter(output.clone(), displayIndex, subjectKeys.length));
                                }
                                ...
                                this[subjectType.propertyStyle](key, valueOutput, subjectIsArrayLike);
                            });
                        });

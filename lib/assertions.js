...
module.exports = expect => {
  ...
  expect.addAssertion("<string> not to match <regexp>", (expect, subject, regexp) =>
    expect.withError(
      () => {
  ...
  expect.addAssertion(
    ...
    (expect, subject, key, expectedPropertyValue) =>
      expect(subject, "to have [own] property", key).then(actualPropertyValue => {
  ...
  expect.addAssertion("<object> to [not] [only] have keys <array>", (expect, subject, keys) => {
    const keysInSubject = {};
    const subjectKeys = expect.findTypeOf(subject).getKeys(subject);
  ...
  expect.addAssertion("<string> [not] to contain <string+>", function(expect, subject) {
    ...
    expect.withError(
      () => {
        args.forEach(arg => {
          expect(subject.includes(arg), "[not] to be truthy");
        });
      },
      e => {
        expect.fail({
          diff: function(output) {
            ...
            if (expect.flags.not) {
              subject.replace(
                new RegExp(args.map(arg => utils.escapeRegExpMetaChars(arg)).join("|"), "g"),
                ($0, index) => {
                  flushUntilIndex(index);
                  lastIndex += $0.length;
                  output.removedHighlight($0);
              ...
              ranges.sort((a, b) => a.startIndex - b.startIndex).forEach(range => {
                ...
                const firstUncoveredIndex = Math.max(range.startIndex, lastIndex);
  ...
  expect.addAssertion("<array-like> [not] to contain <any+>", function(expect, subject) {
    ...
    expect.withError(
      () => {
        args.forEach(arg => {
          expect(
            subject && Array.prototype.some.call(subject, item => expect.equal(item, arg)),
            "[not] to be truthy"
          );
        });
      },
  ...
  expect.addAssertion("<function> to error [with] <any>", (expect, subject, arg) =>
  ...
  expect.addAssertion("<function> to have arity <number>", (expect, {length}, value) => {
      ...
      return expect.withError(
        ...
        err => {
          expect.fail({
            ...
            diff: function(output) {
              const diff = err.getDiff({ output: output });
              diff.inline = true;
              return diff;
            }
          });
      ...
      return expect.promise
        .any(
          keys.map((key, index) => {
            ...
            return expect.promise(() =>
              expect(subject[key], "to [exhaustively] satisfy", expected)
            );
          })
        )
        .catch(e =>
  ...
  expect.addAssertion(
    ...
    (expect, {message}, value) => expect(message, "to [exhaustively] satisfy", value)
  );

  expect.addAssertion("<Error> to [exhaustively] satisfy <any>", (expect, {message}, value) => expect(message, "to [exhaustively] satisfy", value)
  );
    ...
    (expect, subject, value) => {
      ...
      valueKeys.forEach(valueKey => {
        keyPromises[valueKey] = expect.promise(() => {
          const valueKeyType = expect.findTypeOf(value[valueKey]);
                ...
                .promise((resolve, reject) => {
                  arrayChangesAsync(
                    ...
                    function equal(a, b, aIndex, bIndex, cb) {
                      ...
                      expect.promise(() => expect(a, "to [exhaustively] satisfy", b)).then(
                        () => {
                          toSatisfyMatrix[aIndex][bIndex] = true;
                          cb(true);
                        },
              ...
              expect.fail({
                diff: function(output, diff, inspect, equal) {
                  ...
                  const indexOfLastNonInsert = changes.reduce(
                    (previousValue, {type}, index) => type === "insert" ? previousValue : index,
                    -1
                  );
                              ...
                              if (expect.findTypeOf(diffItem.value).is("function")) {
                                this.error("missing: ").property(
                                  ...
                                  output.clone().block(function() {
                                    ...
                                    const promise = keyPromises[diffItem.expectedIndex];
  ...
  expect.addAssertion("<object> to [exhaustively] satisfy <object>", (expect, subject, value) => {
    ...
    if (!subjectIsArrayLike) {
      ...
      keys.forEach(key => {
        if (Object.prototype.hasOwnProperty.call(subject, key) && !subjectKeys.includes(key)) {
      ...
      .all([
        expect.promise(() => {
          if (expect.flags.exhaustively) {
            ...
            const valueKeysWithDefinedValues = keys.filter(key => typeof value[key] !== "undefined");
            const subjectKeysWithDefinedValues = subjectKeys.filter(
              key => typeof subject[key] !== "undefined"
            );
            expect(
          ...
          expect.fail({
            diff: function(output, diff, inspect, equal) {
              ...
              const keys = utils
                ...
                .filter(
                  (
                    key // Skip missing keys expected to be missing so they don't get rendered in the diff
              ...
              keys.forEach((key, index) => {
                output.nl(index > 0 ? 1 : 0).i().block(function() {
                  ...
                  const missingArrayIndex = subjectType.is("array-like") && !(key in subject);
                  ...
                  if (!(key in value)) {
                    ...
                    const keyDiff = conflicting && conflicting.getDiff({ output: output });
                  ...
                  const omitDelimiter = missingArrayIndex || index >= subjectKeys.length - 1;
  ...
  expect.addAssertion("<function> to call the callback without error", (expect, subject) =>
  ...
  expect.addAssertion("<function> to call the callback with error", (expect, subject) =>

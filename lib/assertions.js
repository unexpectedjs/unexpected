...
module.exports = function (expect) {
    ...
    expect.addAssertion('arrayLike', 'to be (the empty|an empty|a non-empty) array', function (expect, subject) {
        expect(subject, this.alternations[0] === 'a non-empty' ? 'not to be empty' : 'to be empty');
    });
    ...
    expect.addAssertion(['string', 'arrayLike'], '[not] to have length', function (expect, subject, length) {
        if (!this.flags.not) {
    ...
    expect.addAssertion(['string', 'arrayLike'], '[not] to be empty', function (expect, subject) {
    ...
    expect.addAssertion('arrayLike', 'to be (a non-empty|an) array whose items satisfy', function (expect, subject) { // ...
        var extraArgs = Array.prototype.slice.call(arguments, 2);
        if (extraArgs.length === 0) {
            throw new Error('Assertion "' + this.testDescription + '" expects a third argument');
        }
    ...
    expect.addAssertion('arrayLike', 'to be an array of', function (expect, subject, itemType) {
        expect(subject, 'to be an array whose items satisfy', 'to be a', itemType.replace(/e?s$/, ''));
    });

    expect.addAssertion('arrayLike', 'to be a non-empty array of', function (expect, subject, itemType) {
    ...
    expect.addAssertion('arrayLike', 'to be an array of (strings|numbers|booleans|arrays|objects|functions|regexps|regexes|regular expressions)', function (expect, subject) {
        expect(subject, 'to be an array of', this.alternations[0]);
    });

    expect.addAssertion('arrayLike', 'to be a non-empty array of (strings|numbers|booleans|arrays|objects|functions|regexps|regexes|regular expressions)', function (expect, subject) {
        expect(subject, 'to be a non-empty array of', this.alternations[0]);
    });

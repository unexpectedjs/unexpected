            ...
            forEach(diffLines, function (part, index) {
                if (part.replaced) {
                    ...
                    forEach(stringDiff.diffWordsWithSpace(oldValue, newValue), function (part) {
                        if (part.added) {
                            forEach(part.value.split(specialCharRegexp), function (text) {
                                if (specialCharRegexp.test(text)) {
                                    newLine.text(escapeChar(text), 'bgGreen, cyan, bold');
                                } else {
                                    newLine.text(text, 'bgGreen, white');
                                }
                            });
                        } else if (part.removed) {
                            forEach(part.value.split(specialCharRegexp), function (text) {
                                if (specialCharRegexp.test(text)) {
                                    oldLine.text(escapeChar(text), 'bgRed, cyan, bold');
                                } else {
                                    oldLine.text(text, 'bgRed, white');
                                }
                            });
                        } else {

        ...
        diff: function (actual, expected, output, diff, inspect) {
            ...
            forEach(diffLines, function (part) {
                if (part.replaced) {
                    ...
                    result.diff.append(oldLine).nl().append(newLine);
                    if (endsWithNewline) {
                        result.diff.nl();
                    }
                } else if (part.added) {
                    result.diff.text(part.value.replace(/^(.)/gm, '+$1'), 'green');
                } else if (part.removed) {
                    result.diff.text(part.value.replace(/^(.)/gm, '-$1'), 'red');
                } else {
                    result.diff.text(part.value.replace(/^(.)/gm, ' $1'));
                }
            });
            result.annotation = output.clone().error('should be: ').append(inspect(expected)).nl().append(result.diff);
            return result;
        }
    });

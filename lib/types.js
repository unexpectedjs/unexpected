...
module.exports = function (expect) {
    ...
    expect.addType({
        ...
        equal: function (a, b, equal) {
            ...
            var aUnwrapped = aType.unwrap(a);
            var bUnwrapped = bType.unwrap(b);

            var actualKeys = aType.getKeys(aUnwrapped).filter(function (key) {
                return typeof aUnwrapped[key] !== 'undefined';
            });
            var expectedKeys = expect.findTypeOf(b).getKeys(bUnwrapped).filter(function (key) {
                return typeof bUnwrapped[key] !== 'undefined';
            });
            ...
            for (var j = 0; j < actualKeys.length; j += 1) {
                ...
                if (!equal(aUnwrapped[key], bUnwrapped[key])) {
                    return false;
                }
            }
            return true;
        ...
        inspect: function (obj, depth, output, inspect) {
            var unwrappedObj = this.unwrap(obj);
            var keys = this.getKeys(unwrappedObj);
            if (keys.length === 0) {
            ...
            var inspectedItems = keys.map(function (key, index) {
                var propertyDescriptor = Object.getOwnPropertyDescriptor && Object.getOwnPropertyDescriptor(unwrappedObj, key);
                ...
                var value = (hasSetter && !hasGetter) ? hasSetter : unwrappedObj[key];
        ...
        diff: function (actual, expected, output, diff, inspect, equal) {
            ...
            var unwrappedActual = actualType.unwrap(actual);
            var unwrappedExpected = expectedType.unwrap(actual);
            var keyIndex = {};
            var actualKeys = actualType.getKeys(unwrappedActual);
            actualKeys.concat(expectedType.getKeys(unwrappedExpected)).forEach(function (key) {
                if (!(key in keyIndex)) {
                    keyIndex[key] = key;
                }
            });
            ...
            keys.forEach(function (key, index) {
                output.nl(index > 0 ? 1 : 0).i().block(function () {
                    ...
                    var conflicting = !equal(unwrappedActual[key], unwrappedExpected[key]);
                    var isInlineDiff = false;
                    if (conflicting) {
                        if (!(key in unwrappedExpected)) {
                            annotation.error('should be removed');
                            isInlineDiff = true;
                        } else if (!(key in unwrappedActual)) {
                            this.error('// missing').sp();
                            valueOutput = output.clone().appendInspected(unwrappedExpected[key]);
                            isInlineDiff = true;
                        } else {
                            var keyDiff = diff(unwrappedActual[key], unwrappedExpected[key]);
                            if (!keyDiff || (keyDiff && !keyDiff.inline)) {
                                annotation.shouldEqualError(unwrappedExpected[key]);
                                if (keyDiff) {
                    ...
                    if (!valueOutput) {
                        valueOutput = inspect(unwrappedActual[key], conflicting ? Infinity : null);
                    }
    ...
    expect.addType({
        ...
        equal: function (a, b, equal) {
            if (a === b) {
                ...
                var aType = expect.findTypeOf(a);
                var bType = expect.findTypeOf(b);
                var aItems = aType.unwrap(a);
                var bItems = bType.unwrap(b);
        ...
        diff: function (actual, expected, output, diff, inspect, equal) {
            ...
            var expectedType = expect.findTypeOf(expected);
            var unwrappedActual = actualType.unwrap(actual);
            var unwrappedExpected = expectedType.unwrap(expected);
            ...
            var type = this;
            var changes = arrayChanges(unwrappedActual, unwrappedExpected, equal, function (a, b) {

...
module.exports = function (expect) {
    ...
    expect.addType({
        ...
        prefix: function (output, obj) {
            ...
            return output.text(this.drawPrefix);
        },
        drawSuffix: '}',
        suffix: function (output, obj) {
            output.text(this.drawSuffix);
            var constructor = obj.constructor;
            var constructorName = constructor && typeof constructor === 'function' && constructor !== Object && utils.getFunctionName(constructor);
            if (constructorName && constructorName !== 'Object') {
                output.text(')');
        ...
        inspect: function (obj, depth, output, inspect) {
            ...
            var type = this;
            var hasValueOverride = typeof type.valueFromKey === 'function';
            var inspectedItems = keys.map(function (key, index) {
                ...
                if (hasSetter && !hasGetter) {
                    value = hasSetter;
                } else if (hasValueOverride) {
                    value = type.valueFromKey(obj, key);
                } else {
                    value = obj[key];
                }
                ...
                propertyOutput[type.propertyStyle](key, inspectedValue);
        ...
        diff: function (actual, expected, output, diff, inspect, equal) {
            ...
            var keys = utils.uniqueStringsAndSymbols(actualKeys, expectedKeys);
            var prefixOutput = this.prefix(output.clone(), actual);
            output
                .append(prefixOutput)
            ...
            keys.forEach(function (key, index) {
                output.nl(index > 0 ? 1 : 0).i().block(function () {
                    var valueActual = hasValueOverride ? type.valueFromKey(actual, key) : actual[key];
                    var valueExpected = hasValueOverride ? type.valueFromKey(expected, key) : expected[key];
                    var valueOutput;
                    ...
                    if (conflicting) {
                        if (expectedKeys.indexOf(key) === -1) {
                            ...
                            var keyDiff = diff(valueActual, valueExpected);
                    ...
                    this[type.propertyStyle](key, valueOutput);
    ...
    expect.addType({
        ...
        retrieveKeys: function (obj) {
            var hasSymbols = Object.getOwnPropertySymbols !== undefined;
            if (hasSymbols && obj[Symbol.iterator] !== undefined) {
                return Array.from(obj.keys());
            } else {
                var hasOwnKeysMethod = typeof obj.keys === 'function';
                return (hasOwnKeysMethod ? obj.keys() : Object.keys(obj));
            }
        },
        valueFromKey: function (obj, key) {
            return obj.get(key);
        }
    });

...
module.exports = function(expect) {
    ...
    expect.addType({
        ...
        inspect: function(obj, depth, output, inspect) {
            ...
            var maxLineLength = output.preferredWidth -
                (depth === Infinity ? 0 : depth) * 2 -
                2;
            var width = 0;
            var compact = inspectedItems.length > 5 ||
        ...
        diff: function(actual, expected, output, diff, inspect, equal) {
            if (actual.constructor !== expected.constructor) {
                return output
                    ...
                    .text(
                        (actual.constructor &&
                            utils.getFunctionName(actual.constructor)) ||
                            actual.constructor
                    )
                    .text(' should be ')
                    .text(
            ...
            var keys = utils.uniqueStringsAndSymbols(
                actualKeys,
    ...
    expect.addType({
        ...
        equal: function(a, b, equal) {
            if (a === b) {
                ...
                a.constructor === b.constructor && a.length === b.length
            ) {
                var i;
                if (this.numericalPropertiesOnly) {
        ...
        inspect: function(arr, depth, output, inspect) {
            ...
            var maxLineLength = output.preferredWidth -
                ...
                2;
            var width = 0;
            var multipleLines = this.forceMultipleLines ||
        ...
        diff: function(actual, expected, output, diff, inspect, equal) {
            ...
            if (Math.max(actual.length, expected.length) > this.diffLimit) {
                output.jsComment(
                    'Diff suppressed due to size > ' + this.diffLimit
                );
                return output;
            }
    ...
    expect.addType({
        ...
        unwrap: function(value) {
            return this.getKeys(value).reduce(
                function(result, key) {
    ...
    expect.addType({
        ...
        inspect: function(obj, depth, output) {
            ...
            for (
                var i = 0;
                i < Math.min(this.hexDumpWidth, obj.length);
                i += 1
            ) {
                if (i > 0) {
                    codeStr += ', ';

...
module.exports = function (expect) {
    expect.addType({
        ...
        diff: function (actual, expected, output, diff, inspect) {
            ...
            if (!result.conflict) {
                output.append(expect.inspect(actual, 2));
                return result;
            }

            var comment = output.clone().sp().error('//').sp();
            var propertyOutputs = map(diffedItems, function (comparison, index) {
                var key = comparison.key;
                var propertyOutput = output.clone();
                propertyOutput.key(key);
                propertyOutput.text(':');

                propertyOutput.sp().append(comparison.text);
                ...
                if (comparison.annotation) {
                    propertyOutput.block(comparison.annotation.prependLinesWith(comment));
                }
            ...
            forEach(propertyOutputs, function (propertyOutput, index) {
                output.i().block(propertyOutput).nl();
            });
    ...
    expect.addType({
        ...
        diff: function (actual, expected, output, diff, inspect) {
            ...
            var comment = output.clone().sp().error('//').sp();
            var itemOutputs = map(diffedItems, function (comparison, index) {
                var itemOutput = output.clone();

                itemOutput.append(comparison.text);
                ...
                if (comparison.annotation) {
                    itemOutput.block(comparison.annotation.prependLinesWith(comment));
                }
                return itemOutput;
            });
            ...
            forEach(itemOutputs, function (itemOutput, index) {
                output.i().block(itemOutput).nl();
            });

            for (var i = actual.length; i < expected.length; i += 1) {
                output.i().error('// index ').error(i).error(' missing: ').block(inspect(expected[i])).nl();
            }
    ...
    expect.addType({
        ...
        diff: function (a, b, output, diff) {
            var comparison = diff(extend({
                message: a.message
            }, a), extend({
                message: b.message
            }, b), output);
            comparison.text = output.text('[Error: ').append(comparison.text).text(']');
            return comparison;
        }
    });
    ...
    expect.addType({
        ...
        diff: function (actual, expected, output, diff, inspect) {
            ...
            result.annotation.error('should be: ').append(inspect(expected)).nl();

            result.annotation.blue('Diff:').nl();
            forEach(diffLines, function (part) {
                if (part.replaced) {
                    var oldLine = output.clone();
                    var newLine = output.clone();
                    var endsWithNewline = part.oldValue.slice(-1) === '\n';
                    ...
                    forEach(stringDiff.diffChars(oldValue, newValue), function (part) {
                        if (part.added) {
                            newLine.text(part.value, 'inverse, green');
                        } else if (part.removed) {
                            oldLine.text(part.value, 'inverse, red');
                        } else {
                    ...
                    result.annotation.append(oldLine).nl().append(newLine);
                    if (endsWithNewline) {
                        result.annotation.nl();
                    }
                } else if (part.added) {
                    result.annotation.text(part.value.replace(/^(.)/gm, '+ $1'), 'green');
                } else if (part.removed) {
                    result.annotation.text(part.value.replace(/^(.)/gm, '- $1'), 'red');
                } else {
                    result.annotation.sp(2).text(part.value);
                }
            });
            return result;
        }

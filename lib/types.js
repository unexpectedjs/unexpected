...
module.exports = function (expect) {
    ...
    expect.addType({
        ...
        inspect: function (obj, depth, output) {
            output.text('[').text(this.name).sp();
            if (obj.length > this.hexDumpWidth) {
                output.text(this.hexDump(obj, this.hexDumpWidth))
                    .text(' (+').text(obj.length - this.hexDumpWidth).text(')');

            } else {
                output.text(this.hexDump(obj));
            }
            return output.text(']');
        },
        diff: function (actual, expected, output, diff, inspect) {
            return diff(this.hexDump(actual), this.hexDump(expected));
        }
    });
    ...
    [8, 16, 32].forEach(function (numBits) {
        ['Int', 'Uint'].forEach(function (intOrUint) {
            var constructorName = intOrUint + numBits + 'Array',
                ...
                this['console'].log('the constructor', Constructor);

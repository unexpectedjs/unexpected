...
module.exports = function (expect) {
    ...
    expect.addType({
        ...
        inspect: function (functionInvocation, depth, output, inspect) {
            var inspector = Object.create(this.baseType);
            inspector.prefix = function (output) {
                return output.code((functionInvocation.fn.name || 'anonymous') + '(', 'javascript');
            };
            inspector.suffix = function (output) {
                return output.code(')', 'javascript');
            };

            return inspector.inspect(functionInvocation.args, depth, output, inspect);
        },
        equal: function (functionInvocation1, functionInvocation2, equal) {
            return equal(functionInvocation1.fn, functionInvocation2.fn) && equal(functionInvocation1.args, functionInvocation2.args);
        }
    });
};

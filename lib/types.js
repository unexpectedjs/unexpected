...
module.exports = function(expect) {
  ...
  expect.addType({
    ...
    equal: function(a, b, equal) {
      ...
      var actualKeys = this.getKeys(a).filter(key => typeof a[key] !== "undefined"),
        expectedKeys = this.getKeys(b).filter(key => typeof b[key] !== "undefined");
    ...
    inspect: function(obj, depth, output, inspect) {
      ...
      var type = this;
      var inspectedItems = keys.map((key, index) => {
  ...
  expect.addType({
    ...
    inspect: function(arr, depth, output, inspect) {
      ...
      var inspectedItems = keys.map(key => {
        var inspectedValue;
        if (key in arr) {
  ...
  expect.addType({
    ...
    getKeys: function(value) {
      var keys = this.baseType.getKeys(value).filter(key => !errorMethodBlacklist[key]);
    ...
    inspect: function(f, depth, output, inspect) {
      ...
      if (matchSource) {
        ...
        if (/\n/.test(body) && !/\\\n/.test(body)) {
          ...
          body = body.replace(new RegExp("^(?:" + indent.indent + ")+", "mg"), $0 => utils.leftPad("", $0.length / indent.amount * output.indentationWidth, " "));
        }

...
module.exports = function(expect) {
  ...
  expect.addType({
    ...
    equal: function(a, b, equal) {
      ...
      const actualKeys = this.getKeys(a).filter(key => typeof a[key] !== "undefined"), expectedKeys = this.getKeys(b).filter(key => typeof b[key] !== "undefined");
    ...
    inspect: function(obj, depth, output, inspect) {
      ...
      const inspectedItems = keys.map((key, index) => {
        ...
        const value = hasSetter && !hasGetter ? hasSetter : obj[key];
      ...
      const maxLineLength = output.preferredWidth - (depth === Infinity ? 0 : depth) * 2 - 2;
      let width = 0;
      const compact =
    ...
    diff: function(actual, expected, output, diff, inspect, equal) {
      if (actual.constructor !== expected.constructor) {
        return output
          ...
          .text(
            (actual.constructor && utils.getFunctionName(actual.constructor)) || actual.constructor
          )
          .text(" should be ")
          .text(
            (expected.constructor && utils.getFunctionName(expected.constructor)) ||
      ...
      const keys = utils.uniqueStringsAndSymbols(actualKeys, this.getKeys(expected));
      const prefixOutput = this.prefix(output.clone(), actual);
      ...
      keys.forEach((key, index) => {
        output.nl(index > 0 ? 1 : 0).i().block(function() {
          ...
          const conflicting = !equal(actual[key], expected[key]);
          ...
          if (conflicting) {
            if (!(key in expected)) {
              ...
              const keyDiff = diff(actual[key], expected[key]);
    ...
    similar: function(a, b) {
      ...
      const requiredSimilarKeys = Math.round(Math.max(aKeys.length, bKeys.length) / 2);
  ...
  expect.addType({
    ...
    inspect: function({name}, depth, output) {
  ...
  expect.addType({
    ...
    inspect: function(arr, depth, output, inspect) {
      ...
      const maxLineLength = output.preferredWidth - 20 - currentDepth * output.indentationWidth - 2;
      let width = 0;
      const multipleLines =
    ...
    diff: function(actual, expected, output, diff, inspect, equal) {
      ...
      const changes = arrayChanges(
        ...
        !type.numericalPropertiesOnly &&
          utils.uniqueNonNumericalStringsAndSymbols(this.getKeys(actual), this.getKeys(expected))
      );
      const indexOfLastNonInsert = changes.reduce(
        (previousValue, diffItem, index) => (diffItem.type === "insert" ? previousValue : index),
        -1
      );
      ...
      output.arrowsAlongsideChangeOutputs(
        ...
        changes.map((diffItem, index) => {
          const delimiterOutput = type.delimiter(output.clone(), index, indexOfLastNonInsert + 1);
          if (diffItem.type === "moveTarget") {
            ...
            return output.clone().block(function() {
              if (diffItem.type === "moveSource") {
                ...
                this.block(function() {
                  this.property(diffItem.actualIndex, inspect(diffItem.value), true).amend(
                    delimiterOutput
                  );
                });
              } else {
                  ...
                  this.property(
                    ...
                    output.clone().block(function() {
                      if (valueDiff && valueDiff.inline) {
                        ...
                        this.append(
                          inspect(diffItem.value).amend(delimiterOutput.sp())
  ...
  expect.addType({
    ...
    getKeys: function(value) {
      const keys = this.baseType.getKeys(value).filter(key => !errorMethodBlacklist[key]);
    ...
    equal: function(a, b, equal) {
      return a === b || (equal(a.message, b.message) && this.baseType.equal(a, b));
    },
    inspect: function(value, depth, output, inspect) {
  ...
  expect.addType({
    ...
    identify: function(value) {
      return (
        value && typeof value === "object" && value._isUnexpected && this.baseType.identify(value)
      );
    },
    getKeys: function(value) {
      return this.baseType.getKeys(value).filter(key => !unexpectedErrorMethodBlacklist[key]);
    },
  ...
  expect.addType({
    ...
    inspect: function(f, depth, output, inspect) {
      ...
      const source = Function.prototype.toString.call(f).replace(/\r\n?|\n\r?/g, "\n");
      ...
      if (matchSource) {
        ...
        if (/\n/.test(body) && !/\\\n/.test(body)) {
          ...
          body = body.replace(new RegExp(`^(?:${indent.indent})+`, "mg"), ({length}) => utils.leftPad("", length / indent.amount * output.indentationWidth, " ")
          );
        }
  ...
  expect.addType({
    ...
    inspect: function(f, depth, output, inspect) {
      ...
      let orBranch = false;
      f._expectations.forEach((expectation, index) => {
  ...
  [8, 16, 32].forEach(function(numBits) {
    ["Int", "Uint"].forEach(intOrUint => {
      const constructorName = `${intOrUint + numBits}Array`, Constructor = global[constructorName];

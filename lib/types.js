...
module.exports = function (expect) {
    expect.addType({
        ...
        diff: function (actual, expected, output, diff, inspect) {
            ...
            forEach(keys, function (key, index) {
                ...
                if (!valueOutput) {
                    valueOutput = inspect(actual[key]);
                }

                output.i().block(output.clone()
                                 .key(key).text(':').sp()
                                 .block(valueOutput.text(last ? ' ' : ','))
                                 .block(annotation.prependLinesWith(comment))).nl();
            });
    ...
    expect.addType({
        ...
        diff: function (actual, expected, output, diff, inspect) {
            ...
            forEach(actual, function (v, index) {
                var valueOutput;
                var annotation = output.clone();
                if (!expect.equal(v, expected[index])) {
                    var keyDiff = diff(v, expected[index]);
                    if (!keyDiff || (keyDiff && !keyDiff.inline)) {
                        valueOutput = inspect(actual[index]);
                        annotation.error('should be: ')
                            .block(inspect(expected[index]));
                ...
                var last = index === actual.length - 1;
                output.i().block(output.clone()
                                 .append(valueOutput.text(last ? ' ' : ','))
                                 .block(annotation.prependLinesWith(comment))).nl();
            });


            for (var i = actual.length; i < expected.length; i += 1) {
                var annotation = output.clone().error('index ').error(i).error(' missing: ').block(inspect(expected[i]));
                output.i().block(annotation.prependLinesWith(output.clone().error('//').sp())).nl();
            }

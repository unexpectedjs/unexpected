  ...
  expect.addType({
    ...
    equal(a, b, equal) {
      ...
      const actualKeys = this.getKeys(a).filter(
        key => typeof a[key] !== 'undefined'
      );
      const expectedKeys = this.getKeys(b).filter(
        key => typeof b[key] !== 'undefined'
      );
    ...
    diff(actual, expected, output, diff, inspect, equal) {
      ...
      const keys = utils.uniqueStringsAndSymbols(
        actualKeys,
        this.getKeys(expected)
      );
      const prefixOutput = this.prefix(output.clone(), actual);
  ...
  expect.addType({
    ...
    equal(a, b, equal) {
      if (a === b) {
        ...
        if (!this.numericalPropertiesOnly) {
          const aKeys = this.getKeysNonNumerical(a).filter(
            (
              key // include keys whose value is not undefined
            ) => typeof a[key] !== 'undefined'
          );
          const bKeys = this.getKeysNonNumerical(b).filter(
            (
              key // include keys whose value is not undefined on either LHS or RHS
  ...
  expect.addType({
    ...
    inspect(f, depth, output, inspect) {
      ...
      if (matchSource) {
        ...
        if (/\n/.test(body) && !/\\\n/.test(body)) {
          body = body.replace(
            new RegExp(`^ {${bodyIndent.length}}`, 'mg'),
            ''
          );
          ...
          body = body.replace(
            ...
            $0 =>
              utils.leftPad(
                '',
                $0.length / indent.amount * output.indentationWidth,
                ' '
              )
          );
        }
  ...
  expect.addType({
      ...
      for (let i = 0; i < maxLength; i += this.hexDumpWidth) {
        ...
        for (let j = 0; j < this.hexDumpWidth; j += 1) {
          if (i + j < maxLength) {
            const octet = obj[i + j];
            hexChars +=
              `${leftPad(octet.toString(16).toUpperCase(), this.digitWidth, '0')} `;
    ...
    inspect(obj, depth, output) {
      ...
      for (let i = 0; i < Math.min(this.hexDumpWidth, obj.length); i += 1) {
        ...
        const octet = obj[i];
        codeStr +=
          `0x${leftPad(octet.toString(16).toUpperCase(), this.digitWidth, '0')}`;
      }

    ...
    expect.addType({
        ...
        diff: function (actual, expected, output, diff, inspect) {
            ...
            if (actual.indexOf('\n') !== -1 || expected.indexOf('\n') !== -1) {
                ...
                result.annotation.blue('Diff:       ');
                forEach(stringDiff.diffChars(actual, expected), function (part) {
                    if (part.added) {
                        result.annotation.text(part.value, 'bgGreen');
                    } else if (part.removed) {
                        result.annotation.text(part.value, 'bgRed');
                    } else {
                        result.annotation.text(part.value);
                    }
                });
            }
            return result;

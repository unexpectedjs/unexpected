...
module.exports = function (expect) {
    ...
    expect.addType({
        ...
        diff: function (actual, expected, output, diff, inspect) {
            var diff = utils.diffStrings(this.hexDump(actual), this.hexDump(expected), output, 'Chars');
            for (var i = 0 ; i < diff.output.length ; i += 1) {
                var isInAsciiChars = false;
                for (var j = 0 ; j < diff.output[i].length ; j += 1) {
                    var obj = diff.output[i][j];
                    var replacement = '';
                    for (var k = 0 ; k < obj.args[0].length ; k += 1) {
                        ...
                        if (ch === '│') {
                            ...
                            replacement = '';
                            diff.output[i].splice(j, 0, {style: 'text', args: [' ', 'white']})
                            k = 0;
                            ch = ' ';
                            if (leftover.length > 0) {
                                diff.output[i].splice(j + 1, 0, {style: 'text', args: [leftover, obj.args[1]]})
                            }
                            obj = diff.output[i][j];
                        }
                        replacement += ch;

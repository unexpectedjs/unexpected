...
module.exports = function (expect) {
    ...
    expect.addType({
        ...
        getKeys: Object.getOwnPropertySymbols ? function (obj) {
            var keys = Object.keys(this.unwrap(obj));
        ...
        equal: function (a, b, equal) {
            ...
            var unwrappedA = this.unwrap(a);
            var unwrappedB = this.unwrap(b);

            var aKeys = this.getKeys(a).filter(function (key) {
                    return typeof a[key] !== 'undefined';
                }),
                bKeys = this.getKeys(b).filter(function (key) {
            ...
            aKeys.sort(keyComparator);
            bKeys.sort(keyComparator);
            // cheap key test
            ...
            for (var j = 0; j < aKeys.length; j += 1) {
                var key = aKeys[j];
                if (!equal(unwrappedA[key], unwrappedB[key])) {
                    return false;
                }
            }
            return true;
        ...
        diff: function (actual, expected, output, diff, inspect, equal) {
            ...
            output.inline = true;
            var unwrappedActual = this.unwrap(actual);
            var unwrappedExpected = this.unwrap(expected);
            ...
            keys.forEach(function (key, index) {
                output.nl(index > 0 ? 1 : 0).i().block(function () {
                    ...
                    var expectedValue = unwrappedExpected[key];
                    var actualValue = unwrappedActual[key];
                    var conflicting = !equal(actualValue, expectedValue);
                    ...
                    if (conflicting) {
                        if (!(key in unwrappedExpected)) {
                            ...
                            valueOutput = output.clone().appendInspected(expectedValue);
                            isInlineDiff = true;
                        } else {
                            var keyDiff = diff(actualValue, expectedValue);
                            if (!keyDiff || (keyDiff && !keyDiff.inline)) {
                                annotation.shouldEqualError(expectedValue);
                                if (keyDiff) {
                    ...
                    if (!valueOutput) {
                        valueOutput = inspect(actualValue, conflicting ? Infinity : null);
                    }
    ...
    expect.addType({
        ...
        inspect: function (arr, depth, output, inspect) {
            ...
            var unwrappedArr = this.unwrap(arr);
            ...
            var inspectedItems = keys.map(function (key) {
                var inspectedValue;
                if (key in unwrappedArr) {
                    inspectedValue = inspect(unwrappedArr[key]);
        ...
        diff: function (actual, expected, output, diff, inspect, equal) {
            ...
            var type = this;
            var changes = arrayChanges(type.unwrap(actual), type.unwrap(expected), equal, function (a, b) {

...
module.exports = function (expect) {
    expect.addType({
        ...
        diff: function (actual, expected, output, diff, inspect) {
            ...
            if (!result.conflict) {
                output.append(inspect(output.clone(), actual).removeFormatting());
                return result;
            }

            var propertyOutputs = map(diffedItems, function (comparison, index) {
                var style = comparison.conflict && comparison.annotation ? 'error' : 'text';
                var key = comparison.key;
                var propertyOutput = output.clone();
                if (key.match(/["' ]/)) {
                    propertyOutput[style](inspect(key));
                } else {
                    propertyOutput[style](key);
                }
                propertyOutput.text(':');
                ...
                if (comparison.annotation) {
                    propertyOutput.comment(' // ');
                    propertyOutput.block(comparison.annotation);
                }
                return propertyOutput;
            });
    ...
    expect.addType({
        ...
        diff: function (actual, expected, output, diff, inspect) {
            ...
            var itemOutputs = map(diffedItems, function (comparison, index) {
                ...
                if (comparison.annotation) {
                    itemOutput.comment(' // ');
                    itemOutput.block(comparison.annotation);
                }
            });

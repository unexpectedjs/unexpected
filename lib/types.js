  ...
  expect.addType({
    ...
    inspect(obj, depth, output, inspect) {
      ...
      const inspectedItems = keys.map((key, index) => {
        ...
        if (hasSetter && !hasGetter) {
          value = hasSetter;
        } else if (typeof type.valueForKey === 'function') {
    ...
    diff(actual, expected, output, diff, inspect, equal) {
      ...
      const type = this;
      const hasValueForKey = typeof type.valueForKey === 'function';
      keys.forEach((key, index) => {
        output
          .nl(index > 0 ? 1 : 0)
          .i()
          .block(function() {
            var valueActual = hasValueForKey
              ? type.valueForKey(actual, key)
              : actual[key];
            var valueExpected = hasValueForKey
              ? type.valueForKey(expected, key)
              : expected[key];
            let valueOutput;

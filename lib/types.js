        ...
        diff: function (actual, expected, output, diff, inspect) {
            ...
            forEach(keys, function (key, index) {
                output.i().block(function () {
                    ...
                    var conflicting = !expect.equal(actual[key], expected[key]);
                    if (conflicting) {
                        if (!(key in expected)) {
                    ...
                    if (!valueOutput) {
                        valueOutput = expect.inspect(actual[key], conflicting ? Infinity : 1);
                    }
    ...
    function markUpSpecialCharacters(output, text, specialCharStyle, baseStyle) {
        forEach(text.split(specialCharRegexp), function (part) {
            if (specialCharRegexp.test(part)) {
                output.text(escapeChar(part), specialCharStyle);
            } else {
                output.text(part, baseStyle);
            }
        });
        return output;
    }
            ...
            forEach(diffLines, function (part, index) {
                ...
                if (part.replaced) {
                    ...
                    forEach(stringDiff.diffWordsWithSpace(oldValue, newValue), function (part) {
                        if (part.added) {
                            markUpSpecialCharacters(newLine, part.value, 'bgGreen, cyan, bold', 'bgGreen, white');
                        } else if (part.removed) {
                            markUpSpecialCharacters(oldLine, part.value, 'bgRed, cyan, bold', 'bgRed, white');
                        } else {
                    ...
                    if (endsWithNewline) {
                        value = part.value.slice(0, -1);
                    }

                    output.append(function () {
                        markUpSpecialCharacters(this, value, 'green', 'green').prependLinesWith(function () {
                            this.green('+');
                        });
                    });
                    ...
                    output.append(function () {
                        markUpSpecialCharacters(this, value, 'red', 'red').prependLinesWith(function () {
                            this.red('-');
                        });
                    });
